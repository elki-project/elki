...
import experimentalcode.frankenb.algorithms.partitioning.DBIDGridPartition;
...
/**
 * This implementation uses the additional information provided by the
 * {@link DBIDGridPartition} to pair each partition with the partitions which
 * are within a distance of 1.
 * <p/>
 * Note: This implementation will throw an UnsupportedOperationException if the
 * given partitions are not all {@link DBIDGridPartition}s
 * 
...
public class NearFieldPartitionPairing<V extends NumberVector<?, ?>> implements IPartitionPairing<V> {
  ...
  public List<PartitionPairing> makePairings(Relation<? extends V> dataSet, List<DBIDPartition> partitions, int packageQuantity) throws UnableToComplyException {
    ...
    if(partitions.size() < 1)
      throw new RuntimeException("Can't work with 0 partitions!");

    if(logger.isVerbose()) {
      logger.verbose("Add diagonals: " + Boolean.toString(addDiagonal));
    }

    DBIDPartition prototypePartition = partitions.get(0);
    DBIDGridPartition prototypePositionedPartition = castPartition(prototypePartition);
    ...
    Map<Position, DBIDGridPartition> partitionMap = new HashMap<Position, DBIDGridPartition>();
    for(DBIDPartition partition : partitions) {
      DBIDGridPartition positionedPartition = castPartition(partition);
    ...
    for(int i = 0; i < totalSpace; ++i) {
      ...
      if(partitionMap.containsKey(position)) {
        DBIDGridPartition partition = partitionMap.get(position);
        ...
        if(!addDiagonal) {
          ...
          for(int dim = 0; dim < dimensionality; ++dim) {
            ...
            if(partitionMap.containsKey(otherPosition)) {
              DBIDGridPartition otherPartition = partitionMap.get(otherPosition);
          ...
          // 2. we pair with each diagonal partition and with each neighbor
          // partition
  ...
  private DBIDGridPartition castPartition(DBIDPartition partition) {
    if(!(partition instanceof DBIDGridPartition)) {
      throw new UnsupportedOperationException("This pairing only works with PositionedPartitions");
    }
    return (DBIDGridPartition) partition;
  }
