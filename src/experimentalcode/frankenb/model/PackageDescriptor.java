...
public class PackageDescriptor<V> implements Iterable<PartitionPairing> {
  ...
  /**
   * Returns the {@link DynamicBPlusTree} result for the given pairing. If no
   * result is available a new tree is created and returned. To determine if a
   * result is available you should use {@link #hasResult(PartitionPairing)}.
   * 
   * @param pairing
   * @return
   * @throws IOException
   */
  public DynamicBPlusTree<DBID, DistanceList> getResultTreeFor(PartitionPairing pairing) throws IOException {
    Pair<File, File> resultFiles = getResultFilesFor(pairing);
    if(!pairing.hasResult()) {
      throw new RuntimeException("This pairing has no result.");
    }
    return new DynamicBPlusTree<DBID, DistanceList>(new BufferedDiskBackedDataStorage(resultFiles.first), new BufferedDiskBackedDataStorage(resultFiles.second), new ConstantSizeIntegerDBIDSerializer(), new DistanceListSerializer());
  }
  ...
  public DynamicBPlusTree<Integer, DistanceList> createResultTreeFor(PartitionPairing pairing, int maxKeysPerBucket) throws IOException {
    if(pairing.hasResult()) {
      throw new RuntimeException("This pairing already has a result.");
    }
    Pair<File, File> resultFiles = getResultFilesFor(pairing);
    ...
    return new DynamicBPlusTree<Integer, DistanceList>(new BufferedDiskBackedDataStorage(resultFiles.first), new BufferedDiskBackedDataStorage(resultFiles.second), new ConstantSizeIntegerSerializer(), new DistanceListSerializer(), maxKeysPerBucket);
  }
  ...
  public void setResultFor(PartitionPairing pairing, DynamicBPlusTree<DBID, DistanceList> resultTree) throws IOException {
    resultTree.close();
