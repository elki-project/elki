 ...
 * @param <O> Object type
 */
public class KernelOutlier<O extends FeatureVector<?>> extends AbstractDistanceBasedAlgorithm<O, OutlierResult> implements OutlierAlgorithm {
  /**
   * Class logger.
   */
   ...
   */
  public KernelOutlier(DistanceFunction<? super O> distanceFunction, int kmin, int kmax, KernelDensityFunction kernel, double scale, boolean usemad) {
  ...
  public OutlierResult run(Database database, Relation<O> rel) {
    final DistanceQuery<O> dq = database.getDistanceQuery(rel, getDistanceFunction());
    ...
    for(DBIDIter iter = ids.iter(); iter.valid(); iter.advance()) {
      ...
      int k = 1;
      double sum = 0.;
      for(DoubleDBIDListIter kneighbor = neighbors.iter(); kneighbor.valid(); kneighbor.advance(), k++) {
        ...
        for(DoubleDBIDListIter neighbor = neighbors.iter(); neighbor.valid(); neighbor.advance()) {
          double dens = sca * kernel.density(neighbor.doubleValue() * ibw);
    ...
    DoubleRelation scoreres = new MaterializedDoubleRelation("Kernel Outlier Factor", "kof-outlier", kofs, ids);
