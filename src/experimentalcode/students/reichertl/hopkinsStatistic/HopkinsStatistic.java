...
public class HopkinsStatistic<V extends NumberVector<V, ?>, D extends NumberDistance<D, ?>> extends AbstractPrimitiveDistanceBasedAlgorithm<V, D, Result> {
  ...
  public HopkinsResult run(Database database, Relation<V> relation) {
    ...
    MeanVariance h = new MeanVariance();
    MeanVariance u = new MeanVariance();
    MeanVariance w = new MeanVariance();
     ...
     // compute NN distances for random objects from within the database
        ...
        double hopkins = a / (a + b);
        h.put(hopkins);
        u.put(a);
        w.put(b);
      }
      String r = "samplesize " + sampleSize + " dim " + dim + 
          " hopkins mean: " + h.getMean() + " hopkins variance: " + h.getSampleVariance() + 
          " umean "+ u.getMean()+" uvar " +u.getSampleVariance() + " wmean " + w.getMean() + " wvar " + w.getSampleVariance();
   
    result.add(r);
  }
      return new HopkinsResult(result,"dim"+dim);
  }
  ...
  public <T extends V> Collection<V> getUniformObjs(Relation<V> relation, Random masterRandom, int sampleSize) {
    ...
    logger.debug("min " + toString(minima)+" max " + toString(maxima));
    }
