 ...
 * @author Ernst
 */
public class LMCLUS<V extends NumberVector<V,?>> extends AbstractAlgorithm
{
  /**
  ...
  private static final Logging logger = Logging.getLogger(LMCLUS.class);
    ...
    /**
     * The id of the origin used to calculate the linear manifold in the findSeperation Method.
     */
    private DBID origin;
     ...
     * @param samplingLevel
     * @param sensivityThreshold the threshold specifying if a manifold is good enough to be seen as cluster.
     * @return A Clustering Object containing all the clusters found by the algorithm.
     * @throws de.lmu.ifi.dbs.elki.utilities.UnableToComplyException
     */
    private Clustering<Model> runLMCLUS(Database<V> d,int maxLMDim,int samplingLevel,double sensivityThreshold) throws UnableToComplyException{
      Clustering<Model> ret = new Clustering<Model>("LMCLUS Clustering", "lmclus-clustering");
      while (d.size()>NOISE_SIZE){
        Database<V> dCopy = d; //TODO copy database
        int lMDim = 1;
        ...
        DBIDs delete = dCopy.getIDs();
        HashSetModifiableDBIDs all = DBIDUtil.newHashSet(d.getIDs());
    ...
    /**
     * This method samples a number of linear manifolds an tries to determine which the
     * one with the best cluster is.
     * <PRE>
     ...
     * </PRE>
     * @param databasePartition The partition of the database to search for the cluster.
     * @param dimension the dimension of the linear manifold to sample.
     * @param samplingLevel 
     * @return the overall goodness of the separation. The values origin basis and threshold are returned
     *         indirectly over class variables.
     */
    private double findSeparation(Database<V> databasePartition, int dimension, int samplingLevel) {
      double goodness = -1;
      double threshold = -1;
      DBID origin = null;
      Matrix basis = null;
      // determine the number of samples needed, to secure that with a specific probability
      // in at least on sample every sampled point is from the same cluster.
      int samples = (int) Math.min(Math.log(NOT_FROM_ONE_CLUSTER_PROBABILITY)
                            /(Math.log(1-Math.pow((1.0d/samplingLevel),dimension))),(double)databasePartition.size());
      System.out.println("Dimension: " + dimension);;
      System.out.println("Number of samples: " + samples);
      Random r = new Random();
      for(int i = 1; i<= samples;i++){
        System.out.println(i);
        DBIDs sample = databasePartition.randomSample(dimension+1,r.nextLong());
        ...
        DBIDs data = databasePartition.getIDs();
     ...
     * It uses the established Gram-Schmidt algorithm for orthonormalisation:
     * <PRE>
     * u_1 = v_1
    ...
    private double evaluateThreshold(FlexiHistogram<Double,Double> histogramm)
    {
        ...
        double hj = backwards.next().second;
         p1[i] = p1[i-1] + hi;
          if ( p1[i]!=0 )
          {
            ...
            sigma1[i]= ( p1[i-1] * ( sigma1[i-1] + (mu1[i-1]-mu1[i]) * (mu1[i-1]-mu1[i]) )
            + hi * (i - mu1[i]) * (i - mu1[i]) ) / p1[i];
          }
          else
          {
            mu1[i]=0;
      ...
      for(int i = 1; i<jt.length-1;i++)
      {
        ...
        if(devCur>= 0 && devPrev <= 0)
        {
           ...
           double localDepth = 0;
           int leftHeight = i;
           while(leftHeight >=1 && jt[leftHeight-1] >= jt[leftHeight]) leftHeight--;
           int rightHeight = i;
           while(rightHeight < jt.length-1 && jt[rightHeight] <= jt[rightHeight+1]) rightHeight++;
