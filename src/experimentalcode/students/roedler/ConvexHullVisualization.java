  ...
  //  addCSSClasses(svgp);
    ...
    for(int cnum = 0; cnum < clustering.getAllClusters().size(); cnum++) {
      ...
      for (int i = 0; i < clsPoints.length; i++) {
        ...
        double[] projP = proj.fastProjectDataToRenderSpace(database.get(clpnum).getColumnVector());
      ...
      if (chres != null){
        minX = chres[0].get(0); maxX = chres[0].get(0);
        minY = chres[1].get(1); maxY = chres[1].get(1);
        
        for (int i = 0; i < chres.length; i++) {
          if (chres[i].get(0) > maxX) {maxX = chres[i].get(0); }
          if (chres[i].get(0) < minX) {minX = chres[i].get(0); }
          if (chres[i].get(1) > maxY) {maxY = chres[i].get(1); }
          if (chres[i].get(1) < minY) {minY = chres[i].get(1); }
          
          path.drawTo(chres[i].get(0), chres[i].get(1));
        }
        double hullarea = (Math.abs(maxX - minX) * Math.abs(maxY - minY));
        double projarea = (proj.estimateViewport().getFirst().getMax() - proj.estimateViewport().getFirst().getMin()) *
                          (proj.estimateViewport().getSecond().getMax() - proj.estimateViewport().getSecond().getMin());
        
        opacity =  Math.sqrt(((double) clsPoints.length / database.size()) * ((projarea - hullarea) / projarea));
...
*/
   ...
   * the lines between kMeans clusters
   * 
   * @author Heidi Kolb
   * 
