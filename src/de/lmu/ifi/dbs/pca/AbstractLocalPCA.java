...
public abstract class AbstractLocalPCA implements LocalPCA
{
     ...
     * 
     * @param ids
     *            the ids of the objects for which the PCA should be performed
     * @param database
     ...
     *            the database containing the objects
     * @param delta
     *            the threshold for sufficiently small Eigenvalues after
     *            normalization
     */
    ...
    public String[] setParameters(String[] args) throws ParameterException
    {
        ...
        if (optionHandler.isSet(BIG_VALUE_P))
        {
            ...
            try
            {
                ...
                if (big <= 0)
                    throw new WrongParameterValueException(BIG_VALUE_P,
                            bigValueString, BIG_VALUE_D);
            } catch (NumberFormatException e)
            {
                throw new WrongParameterValueException(BIG_VALUE_P,
                        bigValueString, BIG_VALUE_D, e);
            }
        } else
        {
        ...
        {
            String smallValueString = optionHandler
                    .getOptionValue(SMALL_VALUE_P);
            try
            {
                small = Double.parseDouble(smallValueString);
                if (small < 0)
                    throw new WrongParameterValueException(SMALL_VALUE_P,
                            smallValueString, SMALL_VALUE_D);
            } catch (NumberFormatException e)
            {
                throw new WrongParameterValueException(SMALL_VALUE_P,
     ...
     * @param ids
     *            the list of the object ids for which the eigenvalue
