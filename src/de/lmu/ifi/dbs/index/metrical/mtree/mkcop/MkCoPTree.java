        ...
        extends MTree<O, D>
{
     ...
     * 
     * @param fileName
     *            the name of the file storing the MCopTree
     * @param cacheSize
     ...
     * 
     * @param fileName
     *            the name of the file for storing the entries, if this
    ...
    public MkCoPTree(String fileName, int pageSize, int cacheSize,
            DistanceFunction<O, D> distanceFunction, int kmax, List<O> objects)
    {
        super();
        this.k_max = kmax;
    ...
    public void insert(O object)
    {
        throw new UnsupportedOperationException(
                "Insertion of dingle objects is not supported!");
    }
    ...
    public List<QueryResult<D>> reverseKNNQuery(O object, int k)
    {
        ...
        for (Integer id : candidates)
            knnLists.put(id, new KNNList<D>(k, distanceFunction
                    .infiniteDistance()));
    ...
    public String toString()
    {
        ...
        while (enumeration.hasMoreElements())
        {
            ...
            if (!id.isNodeID())
            {
                ...
                // System.out.println(" consApprox = " +
                // Arrays.asList(e.getConservativeKnnDistanceApproximation()));
                // System.out.println(" progrApprox = " +
            ...
            } else
            {
                ...
                if (node.isLeaf())
                {
                    ...
                    // System.out.println("Xxxxxxxxxxxxxx " + e.getObjectID() +
                    // " parent = " + node);
                    // }
                    leafNodes++;
                } else
     ...
     * 
     * @param k
     *            the parametr k of the rknn query
     * @param q
     *            the id of the query object
     * @param result
     *            holds the true results (they need not to be refined)
     * @param candidates
     *            holds possible candidates for the result (they need a
     *            refinement)
     */
    private void doReverseKNNQuery(int k, Integer q,
            List<QueryResult<D>> result, List<Integer> candidates)
    {
        ...
        while (!pq.isEmpty())
        {
            ...
            MkCoPTreeNode<O, D> node = (MkCoPTreeNode<O, D>) getNode(pqNode
                    .getValue().getID());
            ...
            if (!node.isLeaf())
            {
                for (int i = 0; i < node.getNumEntries(); i++)
                {
                    ...
                    // entry.getObjectID()+","+q+") = " +distance);
                    D minDist = entry.getCoveringRadius().compareTo(distance) > 0 ? distanceFunction
                            .nullDistance()
                            : distance.minus(entry.getCoveringRadius());
                    D approximatedKnnDist_cons = entry
                            .approximateConservativeKnnDistance(k,
                                    distanceFunction);
                    // System.out.println("minDist(n_" +
                    // entry.getNodeID()+","+q+") = " +minDist);
                    // System.out.println("approximatedKnnDist_cons(n_" +
            ...
            else
            {
                for (int i = 0; i < node.getNumEntries(); i++)
                {
                    ...
                    if (distance.compareTo(approximatedKnnDist_prog) <= 0)
                    {
                        ...
                        // + " - " + q);
                        // KNNList<DoubleDistance> knn = new
                        // KNNList<DoubleDistance>(k,
                    ...
                    {
                        NumberDistance approximatedKnnDist_cons = entry
                                .approximateConservativeKnnDistance(k,
                                        distanceFunction);
                        double diff = distance.getDoubleValue()
                                - approximatedKnnDist_cons.getDoubleValue();
            ...
            if (id.isNodeID())
            {
                ...
                if (node.isLeaf())
                {
                    for (int i = 0; i < node.getNumEntries(); i++)
                    {
                        ...
                        {
                            D knnDist_cons = entry
                                    .approximateConservativeKnnDistance(k,
                                            distanceFunction);
                            D knnDist_prog = entry
                                    .approximateProgressiveKnnDistance(k,
                                            distanceFunction);
                            ...
                            // System.out.println("\nObject " +
                            // entry.getObjectID());
                            // System.out.println("knnDist_soll["+k+"]" +
                            // knnDist_soll);
                            // System.out.println("knnDist_cons["+k+"]" +
                            // knnDist_cons);
                            // System.out.println("knnDist_prog["+k+"]" +
                            // knnDist_prog);
                            ...
                            if (knnDist_cons.compareTo(knnDist_soll) < 0)
                            {
                                        ...
                                        - knnDist_cons.getDoubleValue()) > 0.000000001)
                                {
                                    ...
                                    msg += "\nknnDist_cons[" + k
                                            ...
                                            + knnDist_soll + "\n" + "in "
                                            + node + " at entry " + entry;
                                    System.out.println(Math.abs(knnDist_soll
                                            .getDoubleValue()
                                            - knnDist_cons.getDoubleValue()));
            ...
            List<D> knnDistances)
    {
        ...
        ApproximationLine knnDistances_node = node
                .conservativeKnnDistanceApproximation(k_max);

        for (int k = 1; k <= k_max; k++)
        {
            D knnDistance_node = knnDistances_node.getApproximatedKnnDistance(
                    k, distanceFunction);
            ...
            {
                if (Math.abs(knnDistance.getDoubleValue())
                        - knnDistance_node.getDoubleValue() > 0.000000001)
                {
                    String msg = "\nknnDistance[" + k + "] > knnDistance_node["
                            + k + "] \n" + knnDistance + " > "
                            + knnDistance_node + "\n" + "in " + node
                            + " (entry " + entry + ")";
    ...
    private void testKNNDistances(MkCoPDirectoryEntry<D> rootID)
    {
        ...
        ApproximationLine knnDistances_soll_cons = node
                .conservativeKnnDistanceApproximation(k_max);

        for (int k = 1; k <= k_max; k++)
        {
            D knnDist_ist_cons = rootID.approximateConservativeKnnDistance(k,
                    distanceFunction);
            D knnDist_soll_cons = knnDistances_soll_cons
                    .getApproximatedKnnDistance(k, distanceFunction);

            if (!knnDist_ist_cons.equals(knnDist_soll_cons))
            {
                if (Math.abs(knnDist_ist_cons.getDoubleValue()
                        - knnDist_soll_cons.getDoubleValue()) > 0.000000001)
                {
                    String msg = "\nknnDist_ist_cons[" + k
                            + "] != knnDist_soll_cons[" + k + "] \n"
    ...
    private TreePath split(TreePath path)
    {
        ...
        MkCoPTreeNode<O, D> newNode = (MkCoPTreeNode<O, D>) node.splitEntries(
                assignments.getFirstAssignments(), assignments
        ...
        parent.addDirectoryEntry(new MkCoPDirectoryEntry<D>(assignments
                .getSecondRoutingObject(), parentDistance2,
            ...
            D secondCoveringRadius)
    {
        ...
        MkCoPTreeNode<O, D> root = new MkCoPTreeNode<O, D>(file, dirCapacity,
                false);
        file.writePage(root);
        ...
        root.addDirectoryEntry(new MkCoPDirectoryEntry<D>(firstPromoted, null,
                oldRoot.getNodeID(), firstCoveringRadius, oldRoot
                        .conservativeKnnDistanceApproximation(k_max)));

        root.addDirectoryEntry(new MkCoPDirectoryEntry<D>(secondPromoted, null,
                newNode.getNodeID(), secondCoveringRadius, newNode
                        .conservativeKnnDistanceApproximation(k_max)));
    ...
    private void insert(List<O> objects)
    {
        ...
        for (O object : objects)
        {
            ...
            knnLists.put(object.getID(), new KNNList<D>(k_max + 1,
                    distanceFunction.infiniteDistance()));
            ...
            MkCoPLeafEntry<D> newEntry = new MkCoPLeafEntry<D>(object.getID(),
                    parentDistance,
                    new ApproximationLine(k_max, Double.NEGATIVE_INFINITY,
                            Double.NEGATIVE_INFINITY), new ApproximationLine(0,
     ...
     * @param entry
     */
    private void batchApproximateKNNDistances(MkCoPEntry<D> entry,
            Map<Integer, KNNList<D>> knnLists)
    {
        ...
        MkCoPTreeNode<O, D> node = (MkCoPTreeNode<O, D>) getNode(((MkCoPDirectoryEntry) entry)
                .getNodeID());
        if (node.isLeaf())
        {
            for (int i = 0; i < node.getNumEntries(); i++)
            {
                ...
                approximateKnnDistances(e,
                        getKNNList(e.getObjectID(), knnLists));
                // System.out.println("batchApproximateKNNDistances object " +
                // e.getObjectID() + ": " +
        ...
        else
        {
            for (int i = 0; i < node.getNumEntries(); i++)
            {
                MkCoPDirectoryEntry<D> e = (MkCoPDirectoryEntry<D>) node
                        .getEntry(i);
                batchApproximateKNNDistances(e, knnLists);
                // System.out.println("batchApproximateKNNDistances node " +
                // e.getNodeID() + ": " +
                // e.getConservativeKnnDistanceApproximation());
            }
        }
        ApproximationLine approx = node
                .conservativeKnnDistanceApproximation(k_max);
     ...
     * 
     * @param knnDistances
     *            TODO: Spezialbehandlung fuer identische Punkte in DB (insbes.
     *            Distanz 0)
     */
            ...
            List<D> knnDistances)
    {
        ...
        // System.out.println("log_kDist(" + log_kDist.length + ") " +
        ...
        ApproximationLine conservative = approximateUpperHull(convexHull,
                log_k, sum_log_k, sum_log_k2, log_kDist, sum_log_kDist,
                sum_log_k_kDist);
        ...
        double err1 = ssqerr(k_0, k_max, log_k, log_kDist, conservative.getM(),
                conservative.getT());
            ...
            double[] log_kDist, double sum_log_kDist, double sum_log_k_kDist)
    {
        ...
        for (int i = 0; i < l; i++)
        {
            ...
            if (!(i > 0 && log_kDist[lowerHull[i - 1]] < log_kDist[lowerHull[i]]
                    ...
                    && !is_right)
                System.out
                        .println("ERROR lower: The bisection search will not work properly !");
            ...
            double[] log_kDist, double sum_log_kDist, double sum_log_k_kDist)
    {
        ...
        for (int i = 0; i < u - 1; i++)
        {
            ...
            ApproximationLine current_approx = new ApproximationLine(k_0,
                    current_m, current_t);
