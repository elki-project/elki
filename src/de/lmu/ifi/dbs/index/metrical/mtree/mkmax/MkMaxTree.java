 ...
 */
public class MkMaxTree<O extends DatabaseObject, D extends Distance<D>> extends
        MTree<O, D>
{
    /**
     * The parameter k.
     ...
     * 
     * @param fileName
     *            the name of the file storing the MTree
     * @param cacheSize
     ...
     * @param k
     *            the parametr k of the knn distance to be stored
     * @param objects
     */
    public MkMaxTree(String fileName, int pageSize, int cacheSize,
            DistanceFunction<O, D> distanceFunction, int k, List<O> objects)
    {
        super();
        this.k = k;
    ...
    public void insert(O object)
    {
        ...
        MkMaxTreeNode<O, D> node = (MkMaxTreeNode<O, D>) path
        ...
        {
            node = (MkMaxTreeNode<O, D>) currentPath.getLastPathComponent()
                    .getIdentifier();
            Integer nodeIndex = currentPath.getLastPathComponent().getIndex();
            MkMaxTreeNode<O, D> parent = (MkMaxTreeNode<O, D>) currentPath
                    .getParentPath().getLastPathComponent().getIdentifier();
            ...
            KNNList<D> knns_q)
    {
        ...
        if (node.isLeaf())
        {
            for (int i = 0; i < node.getNumEntries(); i++)
            {
                ...
                {
                    KNNList<D> knns_p = new KNNList<D>(k, distanceFunction
                            .infiniteDistance());
        ...
        {
            List<DistanceEntry<D>> entries = getSortedEntries(node, q
                    .getObjectID());
    ...
    private TreePath split(TreePath path)
    {
        ...
        parent.addDirectoryEntry(new MkMaxDirectoryEntry<D>(assignments
    ...
    private void insert(List<O> objects)
    {
        ...
        for (O object : objects)
        {
            ...
            knnLists.put(object.getID(), new KNNList<D>(k, distanceFunction
                    .infiniteDistance()));
            ...
            Map<Integer, KNNList<D>> knnLists)
    {
        ...
        MkMaxTreeNode<O, D> node = (MkMaxTreeNode<O, D>) getNode(((MkMaxDirectoryEntry<D>) entry)
                .getNodeID());
