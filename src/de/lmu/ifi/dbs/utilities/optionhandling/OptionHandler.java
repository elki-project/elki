...
public class OptionHandler
{
    ...
    public OptionHandler(Map parameterToDescription, String programCall)
    {
        ...
        {
            String currentOption = OPTION_PREFIX+possibleOptions[i];
            if(currentOption.endsWith(EXPECTS_VALUE))
            {
                this.possibleParameters.put(currentOption.substring(0,currentOption.length()-1), new Boolean(true));
            }
            else
            {
                this.possibleFlags.put(currentOption, new Boolean(true));
            }
        }
    }
