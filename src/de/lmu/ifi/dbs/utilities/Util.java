...
public final class Util
{
     ...
     * @param sep
     *            the seperator between the single values of the float array,
     *            e.g. ','
     * @param digits
     *            the number of fraction digits
            ...
            List<Integer> ids)
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            {
                matrixArray[i][d] = obj.getValue(d + 1).doubleValue()
                        - centroid.getValue(d + 1).doubleValue();
            }
        }
     ...
     * 
     * @param filename
     *            the filename of the transposed file to be plotted
     * @param datasetSize
     *            the size of the transposed data set
     * @param dimensionality
     *            the dimensionality of the transposed data set
    ...
    public static String transposedGnuplotScript(String filename,
            int datasetSize, int dimensionality)
    {
     ...
     */
    public static <T> T instantiate(Class<T> type, String className)
            throws UnableToComplyException
    {
        T instance;
        try
        {
            ...
            {
                // try package of type
                instance = type.cast(Class.forName(
                        type.getPackage().getName() + "." + className)
                        .newInstance());
            }
        ...
        {
            throw new UnableToComplyException(
                    "ClassNotFoundException occurred!", e);
     ...
     * @param separator
     *            the separator to separate entries of the list
     * @param out
     *            the target PrintStream
     */
    public static <O extends Object> void print(List<O> list, String separator,
            PrintStream out)
    {
    ...
    public static SortedSet<ClassLabel> getClassLabels(Database database)
    {
        ...
        {
            labels.add((ClassLabel) database.getAssociation(
                    AssociationID.CLASS, iter.next()));
        }
        return labels;
    }
     ...
     *            the complete array
     * @param part
     *            an array that contains only elements of the first parameter
     * @return an array that contains all elements of the first parameter that
     *         are not contained by the second parameter
     * @throws IllegalArgumentException
     *             if the first array, <code>complete</code> is not as long as
     ...
     */
    public static String[] difference(String[] complete, String[] part)
            throws IllegalArgumentException
    {
        if (complete.length < part.length)
        {
            throw new IllegalArgumentException(
                    "first array must be at least as long as second array.");
        }
        ...
        {
            throw new IllegalArgumentException(
                    "second array contains entries that are not contained by the first array.");
        }
