...
public final class Util {
  ...
  /**
   * Formats the double array d with the specified separator and the
   * specified fraction digits.
   *
   * @param d      the double array to be formatted
   * @param sep    the seperator between the single values of the double array, e.g. ','
   * @param digits the number of fraction digits
   * @return a String representing the double array d
   */
  public static String format(double[] d, String sep, int digits) {
    ...
    for (int i = 0; i < d.length; i++) {
      if (i < d.length - 1)
        buffer.append(format(d[i], digits) + sep);
      else
