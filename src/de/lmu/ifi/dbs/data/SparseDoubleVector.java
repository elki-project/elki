 ...
 * <p/>
 * <p/>
 * Class for storing a DoubleVector as a sparse vector. A SparseDoubleVector only requires storage for
 * those attribute values that are non-zero.
 *
...
public class SparseDoubleVector extends RealVector<Double> {
  ...
  public SparseDoubleVector(Map<Integer, Double> values, int dimensionality) {
    ...
    for (Integer index : values.keySet()) {
      ...
      if (value != 0) this.values.put(index, value);
    }
  ...
  public SparseDoubleVector(double[] values) {
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (value != 0) this.values.put(i, value);
    }
  }
  ...
  public FeatureVector<Double> plus(FeatureVector<Double> fv) {
    if (fv.getDimensionality() != this.getDimensionality()) {
      throw new IllegalArgumentException("Incompatible dimensionality: " + this.getDimensionality() +
                                         " - " + fv.getDimensionality() + ".");
    }
