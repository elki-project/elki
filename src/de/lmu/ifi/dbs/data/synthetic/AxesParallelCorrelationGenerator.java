...
//    maxParameter.setDefaultValue(MAX_DEFAULT);
    ...
    optionHandler.put(NUMBER_P, numberParameter);
  ...
  public String[] setParameters(String[] args) throws ParameterException {
    ...
    // dataDim
    dataDim = (Integer) optionHandler.getParameterValue(dimParameter);

    // corrDim
    corrDim = (Integer) optionHandler.getParameterValue(corrDimParameter);

    // minima
    min = new double[dataDim];
    if (optionHandler.isSet(MIN_P)) {
      List<Double> min_list = (List<Double>) optionHandler.getParameterValue(minParameter);
    ...
    if (optionHandler.isSet(MAX_P)) {
      List<Double> max_list = (List<Double>) optionHandler.getParameterValue(maxParameter);
      for (int i = 0; i < dataDim; i++) {
    ...
    if (optionHandler.isSet(depParameter)) {
      List<Double> dep_list = (List<Double>) optionHandler.getParameterValue(depParameter);
