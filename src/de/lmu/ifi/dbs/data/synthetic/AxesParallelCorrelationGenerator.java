...
import de.lmu.ifi.dbs.utilities.optionhandling.LessEqualGlobalConstraint;
...
public class AxesParallelCorrelationGenerator extends StandAloneWrapper {
	...
	public static final String MIN_D = "<min_1,...,min_d>a comma separated list of the coordinates of the minimum "
			+ "value in each dimension, default is "
			+ MIN_DEFAULT
			+ " in each dimension";
	...
	public AxesParallelCorrelationGenerator() {
		super();
		// parameter dimension
		IntParameter dim = new IntParameter(DIM_P, DIM_D,
				new GreaterConstraint(0));
		optionHandler.put(DIM_P, dim);

		// parameter correlation dim
		IntParameter corr = new IntParameter(CORRDIM_P, CORRDIM_D,
				new GreaterConstraint(0));
		...
		//parameter number
		optionHandler.put(NUMBER_P, new IntParameter(NUMBER_P, NUMBER_D,
				new GreaterConstraint(0)));
		...
		optionHandler.setGlobalParameterConstraint(new LengthConstraint(min,dim));
		optionHandler.setGlobalParameterConstraint(new LengthConstraint(max,dim));
	...
	public static void main(String[] args) {
		...
		try {
			...
			wrapper.exception(wrapper.optionHandler.usage(e.getMessage()),
					cause);
		} catch (Exception e) {
			e.printStackTrace();
	...
	void generateCorrelation(OutputStreamWriter outStream) throws IOException {
		...
		Progress progress_1 = new Progress("Generate the feature vectors",
				number);
		if (isVerbose()) {
		...
		if (jitter != 0) {
			Progress progress_2 = new Progress("Jitter the feature vectors",
					number);
			if (isVerbose()) {
				verbose(LINE_SEPARATOR + "Jitter the feature vectors"
						+ LINE_SEPARATOR);
			}
			for (int n = 0; n < number; n++) {
				for (int d = 0; d < dataDim; d++) {
					double jitter = featureVectors[n][d] * RANDOM.nextDouble()
							* this.jitter / 100;
		...
		if (isVerbose()) {
			verbose(LINE_SEPARATOR + "Print the feature vectors"
					+ LINE_SEPARATOR);
		}
		for (int n = 0; n < number; n++) {
			for (int d = 0; d < dataDim; d++) {
	...
	public String[] setParameters(String[] args) throws ParameterException {
		...
		try {
			...
			if (corrDim <= 0)
				throw new WrongParameterValueException(CORRDIM_P,
						corrdimString, CORRDIM_D);
		} catch (NumberFormatException e) {
			throw new WrongParameterValueException(CORRDIM_P, corrdimString,
					CORRDIM_D, e);
		}
		...
		if (optionHandler.isSet(MIN_P)) {
			...
			if (minima.length != dataDim)
				throw new WrongParameterValueException("Value of parameter "
						+ MIN_P + " has not the specified dimensionality  "
						+ DIM_P + " = " + dataDim);
			...
			for (int i = 0; i < dataDim; i++) {
				try {
					...
					throw new WrongParameterValueException(MIN_P, minString,
							MIN_D);
				}
			}
		} else {
		...
		if (optionHandler.isSet(MAX_P)) {
			...
			if (maxima.length != dataDim)
				throw new WrongParameterValueException("Value of parameter "
						+ MAX_P + " has not the specified dimensionality  "
						+ DIM_P + " = " + dataDim);
			...
			for (int i = 0; i < dataDim; i++) {
				try {
					...
					throw new WrongParameterValueException(MAX_P, maxString,
							MAX_D);
				}
			}
		} else {
		...
		for (int i = 0; i < dataDim; i++) {
			if (min[i] >= max[i]) {
				throw new WrongParameterValueException("Parameter " + MIN_P
						+ " > " + MAX_P + "!");
			}
		}
		...
		if (optionHandler.isSet(DEPENDENT_VALUES_P)) {
			...
			if (prefVectorString.length != dataDim) {
				throw new WrongParameterValueException("Value of parameter "
						+ DEPENDENT_VALUES_P
						+ " has not the specified dimensionality  " + DIM_P
						+ " = " + dataDim);
			}
				...
				try {
					...
					if (dv[d] == 0) {
						...
						throw new WrongParameterValueException(
								DEPENDENT_VALUES_P, prefString,
								DEPENDENT_VALUES_D);
					}
				} catch (NumberFormatException e) {
					throw new WrongParameterValueException(DEPENDENT_VALUES_P,
							prefString, DEPENDENT_VALUES_D, e);
				}
			}
			if (c != corrDim) {
				throw new WrongParameterValueException(
						"Value of parameter "
								+ DEPENDENT_VALUES_P
								+ " does not correspond with the specified correlation dimensionality  "
								+ "Numbber of zero values " + c + " != "
								+ corrDim);
			}
			dependentValues = dv;
		} else {
			...
			for (int i = 0; i < dataDim - corrDim; i++) {
				dependentValues[i] = RANDOM.nextDouble() * (max[i] - min[i])
						+ min[i];
			}
		}
		...
		try {
			...
			throw new WrongParameterValueException(NUMBER_P, numberString,
					NUMBER_D, e);
		}
		if (number <= 0) {
			throw new WrongParameterValueException(NUMBER_P, numberString,
		...
		if (optionHandler.isSet(JITTER_P)) {
			...
			try {
				...
				if (jitter < 0 || jitter > 1) {
					throw new WrongParameterValueException(JITTER_P,
							jitterString, JITTER_D);
				}
			} catch (NumberFormatException e) {
				throw new WrongParameterValueException(JITTER_P, jitterString,
						JITTER_D, e);
			}
		} else {
