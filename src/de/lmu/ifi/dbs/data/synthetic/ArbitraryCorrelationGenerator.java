...
public class ArbitraryCorrelationGenerator extends AxesParallelCorrelationGenerator {
	...
	public String[] setParameters(String[] args) throws ParameterException {
		...
		if (optionHandler.isSet(BASIS_P)) {
			List<List> basis_lists = (List<List>)optionHandler.getOptionValue(BASIS_P);
			if(basis_lists.size() != corrDim){
			...
			for (int c = 0; c < corrDim; c++) {
				List basisCoordinates = basis_lists.get(c);
//				String[] basisCoordinates = COMMA_SPLIT.split(basisVectors[c]);
				if (basisCoordinates.size() != dataDim)
					throw new WrongParameterValueException("Value of parameter " + BASIS_P + " has not the specified dimensionality  "
							+ DIM_P + " = " + dataDim);

				for (int d = 0; d < dataDim; d++) {
					try {
						b[d][c] = (Double)basisCoordinates.get(d);
					} catch (NumberFormatException e) {
						throw new WrongParameterValueException(BASIS_P, basis_lists.toString(), BASIS_D);
					}
				}
			}
			basis = new Matrix(b);
...
//			basis = new Matrix(b);
	 ...
	 * 
	 * @param featureVector
	 *            the feature vector to be tested
	 ...
	 *            the feature vectors to be written
	 * @param dependency
	 *            the dependeny of the feature vectors
	 * @param std
	 *            the standard deviation of the jitter of the feature vectors
	 * @throws IOException
	 */
