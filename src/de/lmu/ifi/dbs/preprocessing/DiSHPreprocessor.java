...
public class DiSHPreprocessor extends AbstractParameterizable implements PreferenceVectorPreprocessor<RealVector> {
	...
	private static final String CONDITION = "The value of the preference vector in dimension d_i is set to 1 "
			+ "if the epsilon neighborhood contains more than " + MINPTS_P + " " + "points and the following condition holds: "
	...
	public DiSHPreprocessor() {
		...
		// parameter strategy
		StringParameter strat = new StringParameter(STRATEGY_P, STRATEGY_D, new EqualStringConstraint(new String[] {
				Strategy.APRIORI.toString(), Strategy.MAX_INTERSECTION.toString() }));
	 ...
	 * 
	 */
	private BitSet determinePreferenceVector(Database<RealVector> database, Set<Integer>[] neighborIDs, StringBuffer msg)
	 ...
	 * 
	 */
	private BitSet determinePreferenceVectorByApriori(Database<RealVector> database, Set<Integer>[] neighborIDs, StringBuffer msg)
			throws ParameterException, UnableToComplyException {
	 ...
	 */
	private DimensionSelectingDistanceFunction[] initDistanceFunctions(Database<RealVector> database, int dimensionality, boolean verbose,
