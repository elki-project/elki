 ...
 * The parser provides a parameter for parsing the real values as doubles (default) or float.
 * <p/>
 * Several labels may be given per point. A label must not be parseable as double (or float).
...
public class RealVectorLabelParser extends AbstractParser<RealVector> {
    ...
    try {
      for (String line; (line = reader.readLine()) != null; lineNumber++) {
        if (!line.startsWith(COMMENT) && line.length() > 0) {
          ...
          ObjectAndLabels<RealVector> objectAndLabel = new ObjectAndLabels<RealVector>(featureVector, labels);
  ...
  public String description() {
    ...
    description.append("If parameter " + FLOAT_F + " is set, the real values will be parsed as floats, " +
                       "otherwise the real values will be parsed as as doubles (default).");
    description.append("Any substring not containing whitespace is tried to be read as double (or float). " +
