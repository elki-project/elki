...
public class LinearEquation
{
    ...
    /**
     * The level for logging messages. Logging will be enabled, if the level is
     * unequal to <code>Level.OFF<\code>.
     */
     ...
     * 
     * @param a
     *            the matrix of the coefficients of the linear equation system
     * @param b
    ...
    private void triangularForm(int method) throws NullPointerException
    {
        ...
        while (!exitLoop)
        {
            ...
            if (loggerLevel != Level.OFF)
            {
                ...
                msg.append("  *** pivot at (" + pivotRow + "," + pivotCol
                        + ") = " + pivot + "\n");
     ...
     * 
     * @param pos1
     *            the fist position for the permutation
     * @param pos2
     *            the second position for the permutation
     */
    ...
    private void solve(int method) throws NullPointerException
    {
        ...
        for (int i = rank - 2; i >= 0; i--)
        {
            ...
            x[col[i]] = x[col[i]] / coeff[row[i]][col[i]];
        }// end for i
    ...
    public Matrix getEquationMatrix()
    {
        ...
        for (int i = 0; i < m.length; i++)
        {
            ...
            for (int j = 0; j < m[i].length; j++)
            {
                ...
                if (Math.abs(m[i][j]) < Matrix.DELTA)
                    m[i][j] = 0;
            }
        }
         ...
         * System.out.println(equationsToString()); Double[] doubles =
         * m.get(index); for (int i = 0; i < doubles.length; i++) {
         *  } if (doubles[index] == 0 && index != m.size()-1) { m.remove(index);
         * m.add(doubles); } else { index++; break; } }
         */
    ...
    {
        double[][] a = new double[][] { { 1, 2, 3 }, { 4, 6, 5 } };
