...
import de.lmu.ifi.dbs.utilities.optionhandling.EqualNumberGlobalConstraint;
 ...
 */
public class AttributeWiseRealVectorNormalization extends
		AbstractNormalization<RealVector> {
	/**
	...
	public AttributeWiseRealVectorNormalization() {
		...
		global.add(max);
		optionHandler
				.setGlobalParameterConstraint(new EqualNumberGlobalConstraint(
						global));
	}
	 ...
	 */
	public List<ObjectAndAssociations<RealVector>> normalizeObjects(
			List<ObjectAndAssociations<RealVector>> objectAndAssociationsList)
		...
		try {
			...
			for (ObjectAndAssociations<RealVector> objectAndAssociations : objectAndAssociationsList) {
				double[] values = new double[objectAndAssociations.getObject()
						.getDimensionality()];
				for (int d = 1; d <= objectAndAssociations.getObject()
						.getDimensionality(); d++) {
					values[d - 1] = (objectAndAssociations.getObject()
							.getValue(d).doubleValue() - minima[d - 1])
							/ factor(d);
				}

				RealVector normalizedFeatureVector = objectAndAssociationsList
						.get(0).getObject().newInstance(values);
				normalizedFeatureVector.setID(objectAndAssociations.getObject()
						.getID());
				Map<AssociationID, Object> associations = objectAndAssociations
						.getAssociations();
				normalized.add(new ObjectAndAssociations<RealVector>(
						normalizedFeatureVector, associations));
			}
			return normalized;
		} catch (Exception e) {
			throw new NonNumericFeaturesException(
					"Attributes cannot be normalized.", e);
		}
	}
	 ...
	 */
	public List<RealVector> normalize(List<RealVector> featureVectors)
			throws NonNumericFeaturesException {
		...
		if (minima.length == 0 && maxima.length == 0)
			determineMinMax(featureVectors
					.toArray(new RealVector[featureVectors.size()]));
		...
		try {
			...
			for (RealVector featureVector : featureVectors) {
				...
				for (int d = 1; d <= featureVector.getDimensionality(); d++) {
					values[d - 1] = (featureVector.getValue(d).doubleValue() - minima[d - 1])
							/ factor(d);
				}
				RealVector normalizedFeatureVector = featureVectors.get(0)
						.newInstance(values);
	 ...
	 */
	public RealVector restore(RealVector featureVector)
			throws NonNumericFeaturesException {
		if (featureVector.getDimensionality() == maxima.length
				&& featureVector.getDimensionality() == minima.length) {
			double[] values = new double[featureVector.getDimensionality()];
			for (int d = 1; d <= featureVector.getDimensionality(); d++) {
				values[d - 1] = (featureVector.getValue(d).doubleValue()
						* (factor(d)) + minima[d - 1]);
			}
			RealVector restoredFeatureVector = featureVector
					.newInstance(values);
			restoredFeatureVector.setID(featureVector.getID());
			return restoredFeatureVector;
		} else {
			throw new NonNumericFeaturesException(
					"Attributes cannot be resized: current dimensionality: "
							+ featureVector.getDimensionality()
							+ " former dimensionality: " + maxima.length);
		}
	}
	 ...
	 */
	public List<RealVector> restore(List<RealVector> featureVectors)
			throws NonNumericFeaturesException {
		try {
			...
			throw new NonNumericFeaturesException(
					"Attributes cannot be resized.", e);
		}
	}
	 ...
	 */
	public LinearEquationSystem transform(
		...
		for (int i = 0; i < coeff.length; i++)
			for (int r = 0; r < coeff.length; r++) {
				...
				for (int c = 0; c < coeff[0].length; c++) {
					...
					coeff[row[r]][col[c]] = coeff[row[r]][col[c]]
							/ factor(c + 1);
				}
	...
	public String toString(String pre) {
		...
		result.append(pre).append("normalization class: ").append(
				getClass().getName());
		result.append("\n");
		result.append(pre).append("normalization minima: ").append(
				Util.format(minima));
		result.append("\n");
		result.append(pre).append("normalization maxima: ").append(
				Util.format(maxima));
	...
	public String[] setParameters(String[] args) throws ParameterException {
		...
		if (optionHandler.isSet(MINIMA_P) || optionHandler.isSet(MAXIMA_P)) {
			...
			if (minima.length != maxima.length)
				throw new WrongParameterValueException("Parameter " + MINIMA_P
						+ " and " + MAXIMA_P + " "
	...
	private double factor(int dimension) {
		return maxima[dimension - 1] != minima[dimension - 1] ? maxima[dimension - 1]
				- minima[dimension - 1]
	 ...
	 * featureVectors.
	 * 
	 * @param objectAndAssociationsList
	 *            the list of feature vectors and their associtions
	 */
	private void determineMinMax(
			List<ObjectAndAssociations<RealVector>> objectAndAssociationsList) {
		if (objectAndAssociationsList.isEmpty())
			return;
		int dimensionality = objectAndAssociationsList.get(0).getObject()
				.getDimensionality();
		initMinMax(dimensionality);
	 ...
	 *            the dimensionality of the feature vectors to be normalized
	 */
	...
	private void updateMinMax(RealVector featureVector) {
		if (minima.length != featureVector.getDimensionality()) {
			throw new IllegalArgumentException(
					"FeatureVectors differ in length.");
		}
		for (int d = 1; d <= featureVector.getDimensionality(); d++) {
