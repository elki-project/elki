...
public class Matrix implements Cloneable, java.io.Serializable {
  ...
  public Matrix times(Matrix B) {
    if(B.rowdimension != this.columndimension) {
      throw new IllegalArgumentException("Matrix inner dimensions must agree.");
    }
    Matrix X = new Matrix(this.rowdimension, B.columndimension);
    double[][] C = X.getArray();
    // Optimized ala Jama. jik order.
    double[] Bcolj = new double[this.columndimension];
  ...
  public Matrix transposeTimes(Matrix B) {
    ...
    Matrix X = new Matrix(this.columndimension, B.columndimension);
    double[][] C = X.getArray();
    double[] Bcolj = new double[this.rowdimension];
    for(int j = 0; j < B.columndimension; j++) {
      // Make a linear copy of column j from B
  ...
  public Matrix timesTranspose(Matrix B) {
    ...
    Matrix X = new Matrix(this.rowdimension, B.rowdimension);
    double[][] C = X.getArray();
    for(int j = 0; j < B.rowdimension; j++) {
      double[] Browj = B.elements[j];
      // multiply it with each row from A
      for(int i = 0; i < this.rowdimension; i++) {
        double[] Arowi = this.elements[i];
        double s = 0;
