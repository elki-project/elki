...
public class PairCounting {
  ...
  protected PairCounting(ClusterContingencyTable table) {
    ...
    if (table.contingency[table.size1][table.size2 + 1] != tsize || table.contingency[table.size1 + 1][table.size2] != tsize) {
      LoggingUtil.warning("PairCounting F-Measure is not well defined for overlapping and incomplete clusterings. Expected " + tsize + " but found " + table.contingency[table.size1][table.size2 + 1] + "/" + table.contingency[table.size1 + 1][table.size2] + " elements.");
    }
