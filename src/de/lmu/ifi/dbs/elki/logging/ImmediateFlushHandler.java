...
public class ImmediateFlushHandler extends StreamHandler {
  ...
  /**
   * Provides a stream handler flushing each published LogRecord immediately to
   * the designated OutputStream.
   * <p/>
   * The handler sets the given filter and the handler's level reflects the
   * filter's level.
   * 
  ...
  public ImmediateFlushHandler(OutputStream out, Formatter formatter, SelectiveFilter filter) {
    ...
    super.setLevel(filter.getMinLevel());
  }

  /**
   * The given LogRecord is loggable if it is not null and the filter of this
   * handler would treat it as loggable.
   */
  @Override
