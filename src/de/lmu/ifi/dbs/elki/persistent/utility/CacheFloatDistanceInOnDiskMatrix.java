...
import de.lmu.ifi.dbs.elki.distance.distancefunction.DiskCacheBasedFloatDistanceFunction;
 ...
 * 
 */
public class CacheFloatDistanceInOnDiskMatrix<O extends DatabaseObject> extends AbstractWrapper {
  /**
   ...
   * </p>
   */
  private final ClassParameter<DistanceFunction<O,FloatDistance>> DISTANCE_PARAM = new ClassParameter<DistanceFunction<O,FloatDistance>>(DISTANCE_ID, DistanceFunction.class);
   ...
   */
  private DistanceFunction<O,FloatDistance> distance;
    ...
    for(Integer id1 : database) {
      for(Integer id2 : database) {
        if(id2 >= id1) {
          ...
          float d = distance.distance(id1, id2).getValue();
  ...
  public static void main(String[] args) {
    new CacheFloatDistanceInOnDiskMatrix<DatabaseObject>().runCLIWrapper(args);
  }
}
