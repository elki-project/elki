...
public class TestERiCResults {
  ...
  public void testERiCResults() throws ParameterException {
    ...
    Util.addParameter(ericparams,
        COPAC.PARTITION_ALGORITHM_ID, DBSCAN.class.getCanonicalName());
    Util.addParameter(ericparams,
        DBSCAN.MINPTS_ID, Integer.toString(30));
    Util.addParameter(ericparams,
        DBSCAN.EPSILON_ID, Integer.toString(0));
    // ERiC Distance function in DBSCAN:
    Util.addParameter(ericparams,
        DBSCAN.DISTANCE_FUNCTION_ID, ERiCDistanceFunction.class.getCanonicalName());
    Util.addParameter(ericparams,
        ERiCDistanceFunction.DELTA_ID, Double.toString(0.20));
    Util.addParameter(ericparams,
        ERiCDistanceFunction.TAU_ID, Double.toString(0.04));
    // Preprocessing via HiCo:
    Util.addParameter(ericparams,
        COPAC.PREPROCESSOR_ID, KnnQueryBasedHiCOPreprocessor.class.getCanonicalName());
    Util.addParameter(ericparams,
        KnnQueryBasedHiCOPreprocessor.KNN_HICO_PREPROCESSOR_K, Integer.toString(30));
    Util.addFlag(ericparams, PreprocessorHandler.OMIT_PREPROCESSING_ID);
    // PCA
    Util.addParameter(ericparams,
        PCAFilteredRunner.PCA_COVARIANCE_MATRIX, WeightedCovarianceMatrixBuilder.class.getCanonicalName());
    Util.addParameter(ericparams,
        WeightedCovarianceMatrixBuilder.WEIGHT_ID, ErfcWeight.class.getCanonicalName());
    Util.addParameter(ericparams,
        PCAFilteredRunner.PCA_EIGENPAIR_FILTER, RelativeEigenPairFilter.class.getCanonicalName());
    Util.addParameter(ericparams,
        RelativeEigenPairFilter.EIGENPAIR_FILTER_RALPHA, Double.toString(1.50));
    // Set parameters
    ...
    assertTrue(remainingparams.length == 0);
    // run ERiC on database
    eric.run(db);
    HierarchicalCorrelationClusters<DoubleVector> result = (HierarchicalCorrelationClusters<DoubleVector>) eric.getResult();

    // run by-label as reference
    ByLabelHierarchicalClustering<DoubleVector> bylabel = new ByLabelHierarchicalClustering<DoubleVector>();
    bylabel.run(db);
    HierarchicalClusters<HierarchicalPlainCluster<DoubleVector>, DoubleVector> rbl = bylabel.getResult();
    ...
    // So any loss of quality means something isn't quite right with our algorithms.
    double score = measurer.compareDatabases(result, rbl, 1.0);
    System.out.println("ERiC score: "+score);
    assertTrue(score > 0.70210);
}
