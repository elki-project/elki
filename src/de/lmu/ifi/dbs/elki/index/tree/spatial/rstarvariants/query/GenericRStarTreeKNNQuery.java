...
public class GenericRStarTreeKNNQuery<O extends SpatialComparable, D extends Distance<D>> extends AbstractDistanceKNNQuery<O, D> {
   ...
   */
  protected List<DistanceEntry<D, SpatialEntry>> getSortedEntries(AbstractRStarTreeNode<?, ?> node, O q, SpatialPrimitiveDistanceFunction<? super O, D> distanceFunction) {
    ...
    for(int i = 0; i < node.getNumEntries(); i++) {
      ...
      result.add(new DistanceEntry<D, SpatialEntry>(entry, minDist, i));
    }
