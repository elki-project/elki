...
public class DeLiCluNode extends AbstractRStarTreeNode<DeLiCluNode, DeLiCluEntry> {
  ...
  protected void integrityCheckParameters(DeLiCluNode parent, int index) {
    ...
    if(entry.hasHandled() != hasHandled) {
      ...
      throw new RuntimeException("Wrong hasHandled in node " + parent.getID() + " at index " + index + " (child " + entry.getID() + ")" + "\nsoll: " + soll + ",\n ist: " + ist);
    }
    if(entry.hasUnhandled() != hasUnhandled) {
      ...
      throw new RuntimeException("Wrong hasUnhandled in node " + parent.getID() + " at index " + index + " (child " + entry.getID() + ")" + "\nsoll: " + soll + ",\n ist: " + ist);
    }
  }
}