...
public class ConvexHull
{
    ...
    private void determineLowerAndUpperHull(double[] x, double[] y)
    {
        ...
        for (int i = 1; i < x.length; i++)
        {
            ...
            lowerHull[l] = i;
            while (l >= 2
                    && (y[lowerHull[l]] - y[lowerHull[l - 1]])
                            / (x[lowerHull[l]] - x[lowerHull[l - 1]]) <= (y[lowerHull[l - 1]] - y[lowerHull[l - 2]])
                            / (x[lowerHull[l - 1]] - x[lowerHull[l - 2]]))
            {
                // right curved
            ...
            upperHull[u] = i;
            while (u >= 2
                    && (y[upperHull[u]] - y[upperHull[u - 1]])
                            / (x[upperHull[u]] - x[upperHull[u - 1]]) >= (y[upperHull[u - 1]] - y[upperHull[u - 2]])
                            / (x[upperHull[u - 1]] - x[upperHull[u - 2]]))
            {
                // left curved
        ...
        for (int i = 0; i < i; i++)
        {
            ...
            msg.append("  lhull ").append(i).append("=").append(lowerHull[i])
