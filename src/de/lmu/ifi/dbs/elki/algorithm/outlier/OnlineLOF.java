...
public class OnlineLOF<O extends DatabaseObject, D extends NumberDistance<D, ?>> extends LOF<O, D> {
  ...
  void insert(DBIDs ids, Database<O> database, LOFResult lofResult) {
    ...
    WritableDataStore<List<DistanceResultPair<D>>> knnstore1 = (WritableDataStore<List<DistanceResultPair<D>>>) lofResult.getPreproc1().getMaterialized();
    ...
    if(distQuery != reachdistQuery) {
      ...
      // FIXME: Get rid of this cast - make an OnlineKNNPreprocessor?
      WritableDataStore<List<DistanceResultPair<D>>> knnstore2 = (WritableDataStore<List<DistanceResultPair<D>>>) lofResult.getPreproc2().getMaterialized();
