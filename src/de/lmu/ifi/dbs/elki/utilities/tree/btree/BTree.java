...
public class BTree<K extends Comparable<K> & Externalizable, V extends Externalizable> {
  ...
  /**
   * Creates a new BTree with the specified parameters. The BTree will be hold
   * in main memory.
   * 
  ...
  public BTree(int keySize, int valueSize, int pageSize, int cacheSize) {
    ...
    // init the file
    this.file = new MemoryPageFile<BTreeNode<K, V>>(pageSize, cacheSize, new LRUCache<BTreeNode<K, V>>());
  ...
  /**
   * Creates a BTree with the specified parameters from an existing persistent
   * file.
   * 
   * @param cacheSize the size of the cache in Bytes
  ...
  /**
   * Creates a new BTree with the specified parameters. The BTree will be saved
   * persistently in the specified file.
   * 
   * @param keySize the size of a key in Bytes
   * @param valueSize the size of a value in Bytes
   * @param pageSize the size of a page in Bytes
   ...
   * @return the object with the specified key or null if the key does not
   *         exists
   */
  ...
  /**
   * Returns the object with the specified key and marks the node containing the
   * object as dirty.
   * 
   * @param key the key of the object to be returned
   * @return the object with the specified key or null if the key does not
  ...
  /**
   * Returns a string representation of the hierarchy of this BTree. Each node
   * is printed with its id and its values. The root has id 1, all other nodes
   * inherit the id of its parent node and append their index.
   * 
  ...
  /**
   * Writes the content of this BTree to the specified stream. Each data entry
   * is printed in a separate line.
   * 
  ...
  /**
   * Recursive method to search for the object with the specified key in the
   * given subtree. First call should be with the root of this BTree.
   * 
   * @param node a subtree of this BTree
   * @param key the key to search for
   * @return the search result: encapsulates the node and the index of the
   *         object to find
   */
  private SearchResult search(BTreeNode<K, V> node, K key) {
    ...
    int i = 0;
    while((i < node.getNumKeys() - 1) && (key.compareTo(node.getData(i).key) > 0)) {
      i++;
    }
  ...
  /**
   * Writes a string representation of the subtree with the specified root in
  ...
  private void print(BTreeNode<K, V> node, StringBuffer result) {
    if((node == null) || (node.getNumKeys() == 0)) {
      return;
    }
  ...
  /**
   * Writes a string representation of the hierarchy of the subtree with the
   * specified root in the given StringBuffer.
   * 
   * @param node the root of the subtree
  ...
  /**
   * Writes the content of of the subtree with the specified root to the
   * specified stream. Each data entry is printed in a separate line.
   * 
   * @param node the root of the subtree
   * @param outStream the stream to write into
   * @param printer the object printer that provides the print data for each
  ...
  /**
   * Class SearchResult A helper class for saving the search result
   */
  class SearchResult {
  ...
  private int determineOrder(double pageSize, double keySize, double valueSize) {
    ...
    double overhead = 17;
    double childIDs = 4;
    // pagesize = overhead + (m+1) * valueSize + (m+1) * keySize + (m+2) *
    // childIDs
    int m = (int) ((pageSize - overhead - 2 * childIDs - keySize - valueSize) / (valueSize + keySize + childIDs));
    if(m < 2) {
      throw new IllegalArgumentException("Parameter pagesize is chosen " + "to small!");
    }
