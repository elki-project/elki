...
public class BTreeNode<K extends Comparable<K> & Externalizable, V extends Externalizable> extends AbstractPage<BTreeNode<K, V>> {
  ...
  private static Logging logger = Logging.getLogger(BTreeNode.class.getName());
  ...
  BTreeNode(int m, Integer parentID, PageFile<BTreeNode<K, V>> file) {
    super(file);
    if(m <= 0) {
      throw new IllegalArgumentException("Parameter m must be greater than zero, read m=" + m);
    }
    this.m = m;
  ...
  /**
   * Inserts a data object into the right position. This method is only allowed
   * in leaf nodes.
   * 
  ...
  /**
   * Deletes the data object at the specified index in this node. This method is
   * only allowed in leaf nodes.
   * 
   ...
   * This can only be done, if this node is full. The key in the middle is moved
   * up into the parent, the left ones rest in this node, and the right ones go
   * into a new node. The split can be propagated up to the root and then the
   * height of the BTree increases .
   */
  ...
  private void splitRoot() {
    ...
    // root is not a leaf: move the children, set the parent ids and the leaf
    // flags
    if(!isLeaf) {
      for(int i = 0; i <= m; i++) {
  ...
  /**
   * Treatment of an underflow in this node. Try to merge this node with left or
   * right sibling if possible. If not, merge to one node. The merge can
   * continue recursively in parent node. Underflow and merging can propagate up
   * to the root and reduce the number of the levels of this BTree.
   */
  private void underflowTreatment() {
    ...
    BTreeNode<K, V> parent = getFile().readPage(parentID);
    int parentIndex = 0;
    while(parent.childIDs[parentIndex] != this.getID()) {
      parentIndex++;
    }
    if(logger.isDebugging()) {
      msg.append("\n parentNode: ");
  ...
  /**
   * Merges the entries of the child node at the specified index with the
   * entries of one of its sibling nodes.
   * 
  ...
  private void mergeChild(int index) {
    ...
    if(index > 0) {
      ...
      for(int i = leftBrother.numKeys; i >= 0; i--) {
        BTreeData<K, V> shiftData = i == leftBrother.numKeys ? shiftLeft(index - 1, true) : leftBrother.data[i];
      ...
      for(int i = node.numKeys; i >= 0; i--) {
        BTreeData<K, V> shiftData = i == node.numKeys ? shiftLeft(0, true) : node.data[i];
  ...
  /**
   * Moves the data behind the startpos one position right and inserts the
   * specified data into the gap. This method can only be applied to leaf nodes.
   * 
  ...
  /**
   * Moves the data and the children behind the startpos one position right and
   * inserts the specified data and the specified childID into the gap. If the
   * flag rightChild is set, the childID will be inserted at startPos, otherwise
   * it will be inserted at startPos + 1
   * 
   * @param startPos the start position to shift
   * @param data the data to be inserted
   * @param childID the id of the child to be inserted
   * @param rightChild a flag that indicates the position of the childID: if
   *        rightChild is true, the childID will be inserted at startPos,
  ...
  /**
   * Moves the data and the children behind the startpos one position left and
   * returns the removed data.
   * 
   * @param startPos the start position to shift
   * @param leftChild a flag that indicates the position of the childID to be
   *        removed: if leftChild is true, the childID will be deleted at
   *        startPos, otherwise it will be deleted at startPos + 1
