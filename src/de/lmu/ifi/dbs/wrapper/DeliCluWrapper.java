...
public class DeliCluWrapper extends FileBasedDatabaseConnectionWrapper
{
    ...
    {
        super();
        parameterToDescription.put(DeLiClu.MINPTS_P
                + OptionHandler.EXPECTS_VALUE, DeLiClu.MINPTS_D);
        parameterToDescription.put(DeLiCluTreeDatabase.PAGE_SIZE_P
                + OptionHandler.EXPECTS_VALUE, DeLiCluTreeDatabase.PAGE_SIZE_D);
        parameterToDescription
                .put(DeLiCluTreeDatabase.CACHE_SIZE_P
                        + OptionHandler.EXPECTS_VALUE,
                        DeLiCluTreeDatabase.CACHE_SIZE_D);
    ...
    public List<String> getParameters() throws ParameterException
    {
        ...
        // bulk load
        parameters.add(OptionHandler.OPTION_PREFIX
                + DeLiCluTreeDatabase.BULK_LOAD_F);

        // page size
        if (optionHandler.isSet(DeLiCluTreeDatabase.PAGE_SIZE_P))
        {
            parameters.add(OptionHandler.OPTION_PREFIX
                    + DeLiCluTreeDatabase.PAGE_SIZE_P);
            parameters.add(optionHandler
                    .getOptionValue(DeLiCluTreeDatabase.PAGE_SIZE_P));
        }
        ...
        {
            parameters.add(OptionHandler.OPTION_PREFIX
                    + DeLiCluTreeDatabase.CACHE_SIZE_P);
            parameters.add(optionHandler
                    .getOptionValue(DeLiCluTreeDatabase.CACHE_SIZE_P));
        }
