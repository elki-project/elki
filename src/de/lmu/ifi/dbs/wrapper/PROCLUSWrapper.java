...
/**
 * Wrapper class for PROCLUS algorithm. Performs an attribute wise normalization on
...
public class PROCLUSWrapper extends FileBasedDatabaseConnectionWrapper {
  ...
  public String[] setParameters(String[] args) throws ParameterException {
    ...
    // k, dim
    k = optionHandler.getOptionValue(PROCLUS.K_P);
    dim = optionHandler.getOptionValue(PROCLUS.L_P);

    // k_i
    if (optionHandler.isSet(PROCLUS.K_I_P)) {
      k_i = optionHandler.getOptionValue(PROCLUS.K_I_P);
    }
    else {
      k_i = Integer.toString(PROCLUS.K_I_DEFAULT);
    }
