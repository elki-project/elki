...
public class CorrelationDistanceFunction extends AbstractDistanceFunction<DoubleVector> {
  ...
  public String description() {
    ...
    description.append(":\n");
    String preprocessors = PROPERTIES.getProperty(PROPERTY_PREPROCESSOR_CORRELATION_DISTANCE_FUNCTION);
    String[] preprocessorNames = (preprocessors == null ? new String[0] : PROPERTY_SEPARATOR.split(preprocessors));
    for (String preprocessorName : preprocessorNames) {
      try {
        String desc = ((CorrelationDimensionPreprocessor) Class.forName(preprocessorName).newInstance()).description();
        description.append(preprocessorName);
        description.append('\n');
