 ...
 */
public class DeLiClu123<T extends RealVector> extends DistanceBasedAlgorithm<T> {
  /**
    ...
    try {
      ...
      while (numHandled != size) {
        ...
        if (pqNode.getValue().isExpandable) {
          ...
          expandNodes(db, distFunction, nodePair);
        ...
        // pair of objects
          ...
          reinsertExpanded(distFunction, db, dataPair.entry1, parentID);
  ...
  private void expandNodes(DeLiCluTreeDatabase db, SpatialDistanceFunction<T> distFunction,
    ...
    if (node1.isLeaf())
      expandLeafNodes(distFunction, node1, node2);
    else
...
//    System.out.println("expand DataNodes " + node1 + ", " + node2);
    ...
    for (int i = 0; i < numEntries_1; i++) {
      ...
      for (int j = start_j; j < numEntries_2; j++) {
        ...
        // todo
        /*
        Distance reach = Util.max(distance, Util.max(entry1.getKnnDistance(), entry2.getKnnDistance()));
    ...
    for (Integer id : expanded) {
      ...
      for (int i = 0; i < numEntries; i++) {
        if (! node.isHandled(i)) {
          ...
          // todo
        /*
          Distance reach = Util.max(distance, Util.max(pre.getKnnDistance(), o.getKnnDistance()));
