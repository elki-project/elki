...
public class Hough<V extends RealVector<V, ? extends Number>> extends AbstractAlgorithm<ParameterizationFunction> {
	 ...
	 */
	public static final String MINDIM_D = "the minimum dimensionality of the subspaces to be found, " + "default:" + DEFAULT_MINDIM + ".";
	 ...
	 */
	public static final String ADJUSTMENT_D = "flag indicating that an adjustment of the" + "applied heuristic for choosing an interval " + "is performed after an interval is selected, " + "default: no adjustment";
	...
	/**
	 * Flag indicating that an adjustment of the applied heuristic for choosing
	...
	protected void runInTime(Database<ParameterizationFunction> database) throws IllegalStateException {
		...
		try {
			...
			SubspaceClusterMap<V> clusters = doRun(database, progress);
			if (isVerbose()) {
				StringBuffer msg = new StringBuffer();
			...
			result = new HoughResult<V>(database, clusters, noiseDim);
	...
	/**
	 * // todo Returns a description of the algorithm.
	 * 
	 * @return a description of the algorithm
	 */
	public Description getDescription() {
		return new Description("AdvancedHough", "", "", "unpublished :-(");
	}
	...
	/**
	 * Runs the hough algorithm on the specified database, this method is
	 * recursively called until only noise is left.
	 * 
	 * @param database
	 *            the current database to run the hough algorithm on
	 * @param progress
	 *            the progress object for verbose messages
	 * @return a mapping of subspace dimensionalites to clusters
	 * @throws UnableToComplyException
	 */
	private SubspaceClusterMap<V> doRun(Database<ParameterizationFunction> database, Progress progress) throws UnableToComplyException, ParameterException, NonNumericFeaturesException {

		int dim = database.get(database.iterator().next()).getDimensionality();
		SubspaceClusterMap<V> clusterMap = new SubspaceClusterMap<V>(dim);
		...
		while (!heap.isEmpty()) {
			...
			if (dim > minDim + 1) {
				...
				if (adjust) {
					...
					// basis_dim_minus_1 = runDerivator(database, dim, interval,
					// ids);
					// System.out.println("ids " + ids.size());
					// System.out.println("basis (fuer dim " + (dim - 1) + ") "
					// + basis_dim_minus_1);
				...
				if (db.size() != 0) {
					SubspaceClusterMap<V> clusterMap_dim_minus_1 = doRun(db, progress);
	...
	private void initHeap(DefaultHeap<Integer, HoughInterval> heap, Database<ParameterizationFunction> database, int dim, Set<Integer> ids) {
		...
		for (int i = 0; i < numDIntervals; i++) {
			...
			if (intervalIDs != null && intervalIDs.size() >= minPts) {
				// if (intervalIDs != null && intervalIDs.size() >=
				// database.size()) {
	 ...
	 *            the dimensionality of the database
	 * @param basis
	 *            the basis defining the subspace
	 * @param ids
	 *            the ids for the new database
	 * @param database
	 *            the database storing the paramterization functions
	...
	private Database<ParameterizationFunction> buildDB(int dim, Matrix basis, Set<Integer> ids, Database<ParameterizationFunction> database) throws UnableToComplyException {
		...
		// Integer id = it.next();
		// System.out.print("" +
		// Format.format(result.get(id).getPointCoordinates(), " "));
		// System.out.println(" " + database.getAssociation(AssociationID.LABEL,
		// id));
		// }
		// }
	 ...
	 * 
	 * @param basis
	 *            the basis defining he subspace
	 * @param f
	 ...
	 * @param alpha
	 *            the alpha values
	 * @return a basis defining a subspace described by the specified alpha
	 *         values
	 */
	...
	/**
	 * Computes the product of all sinus values of the specified angles from
	 * start to end index.
	 * 
	 * @param start
	 *            the index to start
	 ...
	 * @param alpha
	 *            the array of angles
	 * @return the product of all sinus values of the specified angles from
	 *         start to end index
	 */
	 ...
	 * interval containing the most unprocessed obejcts.
	 * 
	 * @param heap
	...
	/**
	 * Recursive helper method to determine the next ''best'' interval at
	 * maximum level, i.e. the next interval containing the most unprocessed
	 * obejcts
	 * 
	 * @param heap
	 *            the heap storing the intervals
	...
	/**
	 * Determines the minimum and maximum function value of all parametrization
	 ...
	 *         parametrization functions stored in the specified database
	 */
	...
	/**
	 * Runs the derivator on the specified inerval and assigns all points having
	 ...
	 *            the database containing the parametrization functions
	 * @param interval
	 *            the interval to build the model
	 * @param ids
	 ...
	 */
	private Matrix runDerivator(Database<ParameterizationFunction> database, int dim, HoughInterval interval, Set<Integer> ids) throws UnableToComplyException, ParameterException {
		// build database for derivator
		Database<V> derivatorDB = buildDerivatorDB(database, interval);
		...
		V centroid = (V)new DoubleVector(model.getCentroid());
		DistanceFunction<V, DoubleDistance> df = new WeightedDistanceFunction<V>(weightMatrix);
	...
	/**
	 * Builds a database for the derivator consisting of the ids in the
	 * specified interval.
	 * 
	 * @param database
	 *            the database storing the paramterization functions
	 ...
	 * @return a database for the derivator consisting of the ids in the
	 *         specified interval
	 * @throws UnableToComplyException
	 */
	private Database<V> buildDerivatorDB(Database<ParameterizationFunction> database, HoughInterval interval) throws UnableToComplyException {
		// build objects and associations
		...
		for (Integer id : interval.getIDs()) {
			...
			ObjectAndAssociations<V> oaa = new ObjectAndAssociations(v, associations);
			oaas.add(oaa);
		}
