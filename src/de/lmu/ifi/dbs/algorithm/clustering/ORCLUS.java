...
public class ORCLUS extends AbstractAlgorithm<RealVector> implements
        Clustering<RealVector>
{
    /**
     * Parameter k.
     */
    ...
    public String[] setParameters(String[] args) throws ParameterException
    {
        ...
        if (optionHandler.isSet(K_I_P))
        {
            ...
            try
            {
                ...
                if (k_i <= 0)
                {
                    throw new WrongParameterValueException(K_I_P, k_i_String,
                            K_I_D);
                }
            } catch (NumberFormatException e)
            {
     ...
     */
    private Matrix findBasis(Database<RealVector> database, Cluster cluster,
            int dim)
    {
     ...
     */
    private void merge(Database<RealVector> database, List<Cluster> clusters,
            int k_new, int d_new)
    {
        ...
        for (int i = 0; i < clusters.size(); i++)
        {
            for (int j = 0; j < clusters.size(); j++)
            {
                ...
                ProjectedEnergy pe = projectedEnergy(database, c_i, c_j, i, j,
                        d_new);
     ...
     */
    private ProjectedEnergy projectedEnergy(Database<RealVector> database,
            Cluster c_i, Cluster c_j, int i, int j, int dim)
    {
     ...
     */
    private Cluster union(Database<RealVector> database, Cluster c1,
            Cluster c2, int dim)
    {
    ...
    private class ProjectedEnergy implements Comparable<ProjectedEnergy>
    {
        ...
        ProjectedEnergy(int i, int j, Cluster cluster,
                DoubleDistance projectedEnergy)
        {
            this.i = i;
            this.j = j;
