...
public class FLOC<V extends RealVector<V, Double>> extends
     ...
     * </p>
     */
    private final IntParameter ACTION_ORDER_PARAM = new IntParameter(
        "actionOrder", "specifies the order of actions",
        new GreaterEqualConstraint(0), 0, true);
     ...
     * </p>
     */
    private final IntParameter SEED_PARAM = new IntParameter("seed",
        "seed for initial clusters", new GreaterEqualConstraint(1), 1, true);
     ...
     * </p>
     */
    private final IntParameter K_PARAM = new IntParameter("k",
        "indicates how many biclusters should be found",
        new GreaterEqualConstraint(1), 1);
     ...
     * </p>
     */
    private final DoubleParameter INITIAL_ROW_DIM_PARAM = new DoubleParameter(
        "initialRowDim",
     ...
     * </p>
     */
    private final DoubleParameter INITIAL_COL_DIM_PARAM = new DoubleParameter(
        "initialColDim",
     ...
     * </p>
     */
    private final DoubleParameter MISSING_PARAM = new DoubleParameter(
        "missing",
     ...
     * @param cols the set of columns to include in the computation of the mean
     *             of the given row
     ...
     *             the given column
     * @param col  the column index to compute the mean value w.r.t. the given
     ...
     * excludes missing entries from the computation.
     *
     * @param rows the set of rows to include in the computation of the
     *             biclusterMean
     * @param cols the set of columns to include in the computation of the
     *             biclusterMean
     ...
     * columns set to true.
     *
     * @param rows rows belonging to the currently processed bicluster
     * @param cols columns belonging to the currently processed bicluster
     ...
     *
     * @param currRows rows belonging to the currently processed bicluster
     * @param currCols columns belonging to the currently processed bicluster
     * @return todo noemi
     */
    protected List<Double> initiateRowMeans(BitSet currRows, BitSet currCols) {
        // todo noemi z notwendig?
        int z = 0;
        rowMeans = new ArrayList<Double>();
        for (int i = currRows.nextSetBit(0); i >= 0; i = currRows.nextSetBit(i + 1)) {
    ...
    protected List<Double> initiateColMeans(BitSet currRows, BitSet currCols) {
        ...
        for (int j = currCols.nextSetBit(0); j >= 0; j = currCols.nextSetBit(j + 1)) {
     ...
     *
     * @param row row responsible for the change of residue
     * @return the gain obtained by removing/inserting parameter row
     * @param cluster todo noemi
     */
     ...
     *
     * @param col column responsible for the change of residue
     * @return the gain obtained by removing/inserting parameter row
     * @param cluster todo noemi
     */
     ...
     *
     * @param row row to be removed/inserted in a cluster
     ...
     * Identifies the cluster for the greatest gain
     *
     * @param col column to be removed/inserted in a cluster
     ...
     *
     * @param pos  the row/column with the specified gain
     * @param gain the gain of
     */
