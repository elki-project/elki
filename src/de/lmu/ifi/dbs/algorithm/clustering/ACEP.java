...
/**
 * ACEP Algorithm. XXX: remove?
 *
 * @author Elke Achtert (<a
 *         href="mailto:achtert@dbs.ifi.lmu.de">achtert@dbs.ifi.lmu.de</a>)
 */
public class ACEP extends AbstractAlgorithm<RealVector> {
  ...
  protected void runInTime(Database<RealVector> database) throws IllegalStateException {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        Database<RealVector> partitionDB = clustersPlusNoise.getDatabase();
        for (int i = 0; i < clusterAndNoiseArray.length - 1; i++) {
          clusters.put(i, Arrays.asList(clusterAndNoiseArray[i]));
        }
        Map<Integer, Database<RealVector>> clusterDBs = partitionDB.partition(clusters);

        // iterate over each cluster database
        for (int i = 0; i < clusterAndNoiseArray.length - 1; i++) {
          // for (Integer clusterID : clusterDBs.keySet()) {
          Database<RealVector> clusterDB = clusterDBs.get(i);

          if (isVerbose()) {
            System.out.println("\nApply Correlation Analysis on Partition " + partitionID + ", Cluster " + (i + 1) + ": " + clusterDB.size() + " objects");
          }
        ...
        ClustersPlusNoisePlusCorrelationAnalysis r = new ClustersPlusNoisePlusCorrelationAnalysis(clusterAndNoiseArray, partitionDB, correlationAnalysisSolutions.toArray(new CorrelationAnalysisSolution[correlationAnalysisSolutions.size()]), dependencyDerivator.NF);
  ...
  public Description getDescription() {
    return new Description("ACEP", "algorithm without name ;-)", "First it partitions a database according to the correlation dimension of its objects and " + "performs DBSCAN over the partitions. " + "Second an equality-system describing dependencies between attributes " + "in a correlation-cluster is derived.", "unpublished");
  }
  ...
  public String description() {
    ...
    description.append('\n');
    description.append("Remaining parameters are firstly given to the COPAC algorithm and then to the Dependency" + "Derivator algorithm.");
    description.append('\n');
    description.append('\n');
