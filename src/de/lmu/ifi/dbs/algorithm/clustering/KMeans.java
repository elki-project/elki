...
public class KMeans<D extends Distance<D>> extends DistanceBasedAlgorithm<RealVector, D> implements Clustering<RealVector> {
  ...
  protected void runInTime(Database<RealVector> database) throws IllegalStateException {
    ...
    if (database.size() > 0) {
      ...
      for (int i = 0; i < k; i++) {
        RealVector randomVector = (RealVector) randomBase.randomInstance(random);
        try {
   ...
   */
  protected List<RealVector> means(List<List<Integer>> clusters, List<RealVector> means, Database<RealVector> database) {
   ...
   */
  protected List<List<Integer>> sort(List<RealVector> means, Database<RealVector> database) {
