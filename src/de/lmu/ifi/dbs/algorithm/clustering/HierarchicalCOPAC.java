 ...
 */
public class HierarchicalCOPAC extends AbstractAlgorithm<RealVector> {
  /**
      ...
      for (HierarchicalCorrelationCluster child : children) {
        for (Integer parentCorrDim : parentMap.keySet()) {
          ...
          for (HierarchicalCorrelationCluster parent : parents) {
            if (child.getParents().isEmpty() || ! isParent(dimensionality, distanceFunction, parent, child.getParents()))
            {
