...
public abstract class ProjectedClustering extends AbstractAlgorithm<RealVector> implements Clustering<RealVector> {
  ...
  public static final String K_I_D = "positive integer value to specify the multiplier for "
                                     + "the initial number of seeds, default: " + ProjectedClustering.K_I_DEFAULT;
  ...
  public ProjectedClustering() {
    super();
    // parameter k
    optionHandler.put(ProjectedClustering.K_P, new IntParameter(ProjectedClustering.K_P, ProjectedClustering.K_D, new GreaterConstraint(0)));
  ...
  protected void runInTime(Database<RealVector> database) throws IllegalStateException {
    ...
    try {
      ...
      int i = 0;
      for (ProjectedClustering.Cluster c : clusters) {
  ...
  public String[] setParameters(String[] args) throws ParameterException {
    ...
    // k
    String kString = optionHandler.getOptionValue(ProjectedClustering.K_P);
    try {
      k = Integer.parseInt(kString);
      if (k <= 0) {
        throw new WrongParameterValueException(ProjectedClustering.K_P, kString, ProjectedClustering.K_D);
      }
    }
    ...
    // k_i
    if (optionHandler.isSet(ProjectedClustering.K_I_P)) {
      String k_i_String = optionHandler.getOptionValue(ProjectedClustering.K_I_P);
      try {
        k_i = Integer.parseInt(k_i_String);
        if (k_i <= 0) {
          throw new WrongParameterValueException(ProjectedClustering.K_I_P, k_i_String,
                                                 ProjectedClustering.K_I_D);
        }
      }
      catch (NumberFormatException e) {
        throw new WrongParameterValueException(ProjectedClustering.K_I_P, k_i_String,
                                               ProjectedClustering.K_I_D, e);
      }
    }
    else
      k_i = ProjectedClustering.K_I_DEFAULT;
  ...
  private void assign(Database<RealVector> database, List<ProjectedClustering.Cluster> clusters) {
    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < projectedCentroids.length; i++) {
        ...
        DoubleDistance dist = distanceFunction.distance(o_proj,
                                                        projectedCentroids[i]);
   ...
   */
  private Matrix findBasis(Database<RealVector> database, ProjectedClustering.Cluster cluster,
                           int dim) {
   ...
   */
  private void merge(Database<RealVector> database, List<ProjectedClustering.Cluster> clusters,
                     int k_new, int d_new) {
    ...
    for (int i = 0; i < clusters.size(); i++) {
      for (int j = 0; j < clusters.size(); j++) {
        ...
        ProjectedClustering.ProjectedEnergy pe = projectedEnergy(database, c_i, c_j, i, j,
                                                                 d_new);
   ...
   */
  private ProjectedClustering.ProjectedEnergy projectedEnergy(Database<RealVector> database,
                                                              ProjectedClustering.Cluster c_i, ProjectedClustering.Cluster c_j, int i, int j, int dim) {
  ...
  private class ProjectedEnergy implements Comparable<ProjectedClustering.ProjectedEnergy> {
    ...
    ProjectedEnergy(int i, int j, ProjectedClustering.Cluster cluster,
                    DoubleDistance projectedEnergy) {
      this.i = i;
      this.j = j;
