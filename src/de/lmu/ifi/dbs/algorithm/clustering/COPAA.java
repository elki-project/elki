...
public class COPAA extends AbstractAlgorithm<RealVector>
{
     ...
     */
    public static final String PARTITION_DATABASE_CLASS_D = "<classname>database class for each partition - must implement " + Database.class.getName() + ". " + "If this parameter is not set, the databases of the partitions " + "have the same class as the original database.";
    ...
    protected void runInTime(Database<RealVector> database) throws IllegalStateException
    {
        ...
        for(Iterator<Integer> dbiter = database.iterator(); dbiter.hasNext();)
        {
            ...
            Integer corrdim = (Integer) database.getAssociation(AssociationID.LOCAL_DIMENSIONALITY, id);
