 ...
 */
public class ClustersPlusNoisePlusCorrelationAnalysis extends
        ClustersPlusNoise<RealVector>
{
    /**
     ...
     * clusters and remaining noise and a correlation analysis for each cluster
     * 
     ...
     * @param correlationAnalysisSolutions
     *            an array of correlation analysis solutions for each cluster
     * @param nf
     *            number format for output accuracy
     */
    public ClustersPlusNoisePlusCorrelationAnalysis(
            Integer[][] clustersAndNoise, Database<RealVector> db,
            CorrelationAnalysisSolution[] correlationAnalysisSolutions,
            NumberFormat nf)
    {
        super(clustersAndNoise, db);

        if (clustersAndNoise.length == 0
                && correlationAnalysisSolutions.length != 0)
            throw new IllegalArgumentException(
                    "correlationAnalysisSolutions.length must be 0!");

        if (clustersAndNoise.length != correlationAnalysisSolutions.length + 1)
            throw new IllegalArgumentException(
                    "clustersAndNoise.length != correlationAnalysisSolutions.length + 1!");
            ...
            throws NonNumericFeaturesException
    {
        ...
        if (clusterIndex != clustersAndNoise.length - 1)
        {
            ...
            Matrix printSolution = correlationAnalysisSolution
                    .getPrintSolutionMatrix(normalization);
    ...
    /**
     * Returns the clusters of this result. Each array provides the object ids
     * of one cluster.
     * 
    ...
    public Integer[][] getClusterAndNoiseArray()
    {
        ...
        System.arraycopy(clustersAndNoise, 0, clusters, 0,
                clustersAndNoise.length - 1);
        return clusters;
    }
