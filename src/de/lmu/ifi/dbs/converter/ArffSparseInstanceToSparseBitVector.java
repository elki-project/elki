...
/**
 * Converts an arff sparse instance file to a file readable with a SparseBitVectorLabelParser.
 * All lines beginning with @ will be ignored, there is no check if the specified arff file is in a
 * valid format.
 *
...
public class ArffSparseInstanceToSparseBitVector extends AbstractWrapper {
  ...
  /**
   * Initializes the StreamTokenizer used for reading the ARFF sparse instance file.
   *
   ...
   *
   * @param tokenizer the stream tokenizer
   * @throws java.io.IOException if the information is not read
   *                             successfully
   */
  private void writeHeader(StreamTokenizer tokenizer, PrintStream writer) throws IOException {
    // get name of relation
    getFirstToken(tokenizer);
    if (tokenizer.ttype == StreamTokenizer.TT_EOF) {
      throw new IOException("premature end of file" + ", read " + tokenizer.toString());
    }
    if (ArffSparseInstanceToSparseBitVector.ARFF_RELATION.equalsIgnoreCase(tokenizer.sval)) {
      ...
      throw new IOException("keyword " + ArffSparseInstanceToSparseBitVector.ARFF_RELATION + " expected" + ", read " + tokenizer.toString());
    }
    ...
    int index = 0;
    while (ArffSparseInstanceToSparseBitVector.ARFF_ATTRIBUTE.equalsIgnoreCase(tokenizer.sval)) {
      // get attribute name
      getNextToken(tokenizer);
      ...
      if (tokenizer.ttype == StreamTokenizer.TT_WORD) {
        ...
        if (tokenizer.sval.equalsIgnoreCase(ArffSparseInstanceToSparseBitVector.ARFF_ATTRIBUTE_REAL) ||
          ...
          if (tokenizer.nextToken() != StreamTokenizer.TT_EOL) {
            if ((tokenizer.ttype != StreamTokenizer.TT_WORD) &&
                (tokenizer.ttype != '\'') &&
                (tokenizer.ttype != '\"')) {
              throw new IOException("not a valid date format" + ", read " + tokenizer.toString());
            }
            readTillEOL(tokenizer);
          }
          else {
          ...
          throw new IOException("no valid attribute type or invalid enumeration" + ", read " + tokenizer.toString());
        }
        ...
        if (tokenizer.nextToken() != '{') {
          throw new IOException("{ expected at beginning of enumeration" + ", read " + tokenizer.toString());
        }
        while (tokenizer.nextToken() != '}') {
          if (tokenizer.ttype == StreamTokenizer.TT_EOL) {
            throw new IOException("} expected at end of enumeration" + ", read " + tokenizer.toString());
          }
          else {
        ...
        if (attributeValues.size() == 0) {
          throw new IOException("no nominal values found" + ", read " + tokenizer.toString());
        }
    ...
    if (!ArffSparseInstanceToSparseBitVector.ARFF_DATA.equalsIgnoreCase(tokenizer.sval)) {
      throw new IOException("keyword " + ArffSparseInstanceToSparseBitVector.ARFF_DATA + " expected" + ", read " + tokenizer.toString());
    }

    // Check if any attributes have been declared.
    if (attributeTypes.size() == 0) {
      throw new IOException("no attributes declared" + ", read " + tokenizer.toString());
    }
  }
  ...
  private void getLastToken(StreamTokenizer tokenizer, boolean endOfFileOk) throws IOException {
    if ((tokenizer.nextToken() != StreamTokenizer.TT_EOL) &&
      ...
      throw new IOException("end of line expected" + ", read " + tokenizer.toString());
    }
  }
   ...
   *                     successfully
   */
  private boolean writeSparseInstances(StreamTokenizer tokenizer, PrintStream writer) throws IOException {
    ...
    if (tokenizer.ttype == '{') {
      ...
      throw new IOException("{ expected" + ", read " + tokenizer.toString());
    }
  }
   ...
   *                     successfully
   */
  protected boolean writeSparseInstance(StreamTokenizer tokenizer, PrintStream writer) throws IOException {
    int column = 0;
    int maxIndex = -1;
    int index;
    ...
    do {
      ...
      try {
        ...
        throw new IOException("index number expected" + ", read " + tokenizer.toString());
      }

      if (index <= maxIndex) {
        throw new IOException("indices have to be ordered" + ", read " + tokenizer.toString());
      }
      if ((index < 0) ||
          (index >= attributeTypes.size())) {
        throw new IOException("index out of bounds" + ", read " + tokenizer.toString());
      }
      maxIndex = index;
      ...
      if (tokenizer.ttype == '?') {
        throw new IOException("missing values are not supported" + ", read " + tokenizer.toString());
      }
      else {
        // Check if token is valid.
        if (tokenizer.ttype != StreamTokenizer.TT_WORD) {
          throw new IOException("not a valid value" + ", read " + tokenizer.toString());
        }
        int attributeType = attributeTypes.get(index);
        switch (attributeType) {
          case ARFF_ATTRIBUTE_TYPE_NOMINAL:
            // Check if value appears in header.
            int valIndex = nominalValues.get(index).indexOf(tokenizer.sval);
            if (valIndex == -1) {
          ...
          case ARFF_ATTRIBUTE_TYPE_NUMERIC:
            ...
            try {
              ...
              if (value == 1.0) indices.add(index);
              else if (value != 0.0) {
                throw new IOException("1.0 or 0.0 expected" + ", read " + tokenizer.toString());
              }
            }
            catch (NumberFormatException e) {
              throw new IOException("1.0 or 0.0 expected" + ", read " + tokenizer.toString());
          ...
          default:
            throw new IOException("unknown attribute type in column " + column + ", read " + tokenizer.toString());
        }
      }
      column++;
    }
