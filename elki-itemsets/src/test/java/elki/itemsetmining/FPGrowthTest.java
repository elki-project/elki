 ...
 * @since 0.7.5
 */
public class FPGrowthTest extends AbstractFrequentItemsetAlgorithmTest {
  @Test
  public void testMissing() {
    Database db = loadTransactions(UNITTEST + "itemsets/missing1.txt", 4);
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 1).build().autorun(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 0.5).build().autorun(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 3).build().autorun(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 4).build().autorun(db);
      assertEquals("Size not as expected.", 0, res.getItemsets().size());
    }
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 1) //
          .with(FPGrowth.Par.MINLENGTH_ID, 2) //
          .with(FPGrowth.Par.MAXLENGTH_ID, 3) //
          .build().autorun(db);
      assertEquals("Size not as expected.", 10, res.getItemsets().size());
      for(Itemset i : res.getItemsets()) {
        assertEquals("Bad support", 4 - i.length(), i.getSupport());
  ...
  public void testIncreasing() {
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, .5).build().autorun(db);
  ...
  public void testLarge() {
    ...
    FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
        .with(FPGrowth.Par.MINSUPP_ID, 200).build().autorun(db);
