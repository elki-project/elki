...
public class FPGrowthTest extends AbstractFrequentItemsetAlgorithmTest {
  ...
  public void testMissing() {
    Database db = loadTransactions(UNITTEST + "itemsets/missing1.txt", 4);
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 1).build().run(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 0.5).build().run(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 3).build().run(db);
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, 4).build().run(db);
  ...
  public void testIncreasing() {
    ...
    {
      FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
          .with(FPGrowth.Par.MINSUPP_ID, .5).build().run(db);
  ...
  public void testLarge() {
    ...
    FrequentItemsetsResult res = new ELKIBuilder<>(FPGrowth.class) //
        .with(FPGrowth.Par.MINSUPP_ID, 200).build().run(db);
