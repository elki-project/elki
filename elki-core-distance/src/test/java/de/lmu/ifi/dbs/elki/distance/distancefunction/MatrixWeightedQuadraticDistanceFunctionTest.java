...
/**
 * Unit test for matrix weighted Mahalanobis distance.
 *
  ...
  public void testEuclideanConsistency() {
    ...
    for(int i = 0; i < data.length; i++) {
      for(int j = 0; j < data.length; j++) {
        assertEquals("Not consistent at " + i + "," + j, ref.distance(data[i], data[j]), dis.distance(DoubleVector.wrap(data[i]), DoubleVector.wrap(data[j])), 1e-15);
      }
    }
    // Random permutation of rows:
    double[][] mod = VMath.times(weights, 2);
    dis = new MatrixWeightedQuadraticDistanceFunction(mod);
    for(int i = 0; i < data.length; i++) {
      for(int j = 0; j < data.length; j++) {
        assertEquals("Not consistent at " + i + "," + j, 2 * ref.distance(data[i], data[j]), dis.distance(DoubleVector.wrap(data[i]), DoubleVector.wrap(data[j])), 1e-15);
      }
    }

    assertTrue("Equals not recreatable.", dis.equals(new MatrixWeightedQuadraticDistanceFunction(VMath.copy(mod))));
