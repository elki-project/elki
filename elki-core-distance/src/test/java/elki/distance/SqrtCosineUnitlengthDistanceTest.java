...
/**
 * Unit test for unit-length sqrt cosine distance.
 *
 * @author Erich Schubert
 * @since 0.7.5
 */
public class SqrtCosineUnitlengthDistanceTest extends AbstractDistanceTest {
  @Test
  public void testSpatialConsistency() {
    // Also test the builder - we could have just used .STATIC
    SqrtCosineUnitlengthDistance dist = new ELKIBuilder<>(SqrtCosineUnitlengthDistance.class).build();
    basicChecks(dist);
  ...
  public void compareOnUnitSphere() {
    ...
    SqrtCosineUnitlengthDistance cosnorm = SqrtCosineUnitlengthDistance.STATIC;
    SqrtCosineDistance cosfull = SqrtCosineDistance.STATIC;
    for(int i = 0; i < data.length; i++) {
  ...
  public void testNotCosine() {
    ...
    SqrtCosineUnitlengthDistance cosnorm = new ELKIBuilder<>(SqrtCosineUnitlengthDistance.class).build();
    SqrtCosineDistance cosfull = new ELKIBuilder<>(SqrtCosineDistance.class).build();
    assertEquals("Cosine not ok", 0, cosfull.distance(d1, d2), 0);
    assertEquals("Length not ignored", 1.0, cosnorm.distance(d1, d2), 0);
  }
}
