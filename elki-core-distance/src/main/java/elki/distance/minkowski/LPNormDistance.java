 ...
 * {@link ManhattanDistance}, for p = 2 the standard
...
@Alias({ "lp", "minkowski", "p" })
public class LPNormDistance implements SpatialPrimitiveDistance<NumberVector>, NumberVectorDistance<NumberVector>, Norm<NumberVector> {
  /**
      ...
      return p == (double) (int) p ? // Integer test
          (p == 1. ? ManhattanDistance.STATIC : //
              p == 2. ? EuclideanDistance.STATIC : //
                  new LPIntegerNormDistance((int) p) //
          ) : p == Double.POSITIVE_INFINITY ? MaximumDistance.STATIC : //
              new LPNormDistance(p);
    }
  }
}
