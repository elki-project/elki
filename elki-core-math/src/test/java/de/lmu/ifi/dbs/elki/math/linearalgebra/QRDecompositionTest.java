 ...
 * 
 * As reference if the QR Decomposition is working correctly the QR decomposition functions of python3 numpy 1.12.1-3 package,
...
public final class QRDecompositionTest {
  ...
  private static final double[][] R_NUMPY = { //
      {-176.47662735,  -10.73796586,    2.31758736,  -96.14870963,  -65.41376143,   -4.90149893,  -38.36202052,  127.0366526 ,   54.08081593,  -45.5414415 },
      {   0.        , -205.15529749,   26.48780652,  -14.04574229,  -47.96653746,  -15.32189473, -110.60923218, -102.96646247,  -79.63098275,   36.79662449},
      {   0.        ,    0.        ,  242.23340995,  -48.86661202,  -38.631622  ,   -1.07663499,  -19.27602277,  -45.29954314,   10.94363098,   14.81993916},
      {   0.        ,    0.        ,    0.        , -127.10309591,  -57.78009643,  -23.97981176,   24.49194055,   52.96853963,   42.42144841,   37.66796299},
      {   0.        ,    0.        ,    0.        ,    0.        , -143.28052672,   -4.28033246,  -51.80422555,  -26.96481647,  -43.89216604,   61.25782548},
      {   0.        ,    0.        ,    0.        ,    0.        ,    0.        , -190.86828728,    5.3332843 ,   64.59420421,  -13.5860553 ,   18.58871992},
      {   0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,  124.72133429,   28.31474906,   71.18551349,   -4.73561384},
      {   0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,  131.75600232,  -36.47948289, -139.16223542},
      {   0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        , -125.1023439 ,   26.40411814},
      {   0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,    0.        ,  190.5011458 }};
  ...
  public void testGetQ() {
    ...
    assertTrue(almostEquals(Q_OCTAVE, Q_NUMPY, 1E-6));

    final QRDecomposition QRD = new QRDecomposition(M);
  
    assertTrue(almostEquals(QRD.getQ(), Q_OCTAVE, 2E-6));
    assertTrue(almostEquals(QRD.getQ(), Q_NUMPY, 2E-6));
  }
  ...
  public void testGetR() {
    // assert that octave and numpy functions have almost same results
    assertTrue(almostEquals(R_OCTAVE, R_NUMPY, 1E-6));
    
    final QRDecomposition QRD = new QRDecomposition(M);
    
    assertTrue(almostEquals(QRD.getR(), R_OCTAVE, 2E-6));
    assertTrue(almostEquals(QRD.getR(), R_NUMPY, 2E-6));
  }
  ...
  public void testIsFullRank() {
    ...
    final QRDecomposition QRD_m1 = new QRDecomposition(m1);
    assertTrue(QRD_m1.isFullRank());
    ...
    final QRDecomposition QRD_m2 = new QRDecomposition(m2);
    assertFalse(QRD_m2.isFullRank());
  }
  ...
  public void testSolve() {
    ...
    final double[][] lsq_octave = { //
        {  0.01076018062819501  }, //
        { -0.005427983208297543 }, //
        { -0.001572290955308337 }, //
        {  0.01219551340160215  }, //
        { -0.0009912695684353332}, //
        {  0.004877478839811684 }, //
        { -0.01700610419895007  }, //
        {  0.01094414459995209  }, //
        {  0.001761189365626154 }, //
        {  0.0002632220376317611}, //
    };
    ...
    assertTrue(almostEquals(QRD.solve(B), lsq_octave, 2E-6));
    assertTrue(almostEquals(QRD.solve(B), lsq_numpy, 2E-6));
  }
  ...
  /**
   * Testing that the solve method of the QRDDecomposition class raises an exception if
   * IsFullRank returns false.
   */
  @Test
  public void testSolveIsFullRank() {
    thrown.expect(ArithmeticException.class);
    thrown.expectMessage("Matrix is rank deficient.");
