...
public class NearestNeighborConsistency<O> implements Evaluator {
  ...
  /**
   * Calculate (absolute) kNN consistency for all datapoints and determines the
   * ratio.
   * 
  ...
  public double evaluateClustering(Clustering<?> clustering, Relation<O> relation) {
    ...
    KNNSearcher<DBIDRef> knnQuery = new QueryBuilder<>(relation, distance).kNNByDBID(kPlus1);
