...
public class RadiusDistance implements BIRCHDistance {
  ...
  public double squaredDistance(NumberVector nv, ClusteringFeature cf1) {
    ...
    final int dim = cf1.mean.length;
    final double div = 1. / (cf1.n + 1.);
    double sum = 0.;
    for(int i = 0; i < dim; i++) {
      sum += cf1.n / (cf1.n + 1.) * ((cf1.centroid(i) - nv.doubleValue(i)) * (cf1.centroid(i) - nv.doubleValue(i)));
    }
    sum += cf1.ssd;
  ...
  public double squaredDistance(ClusteringFeature cf1, ClusteringFeature cf2) {
    ...
    double sum = 0.;
    for(int i = 0; i < dim; i++) {
      sum += (n1 * n2) / (n12) * ((cf1.centroid(i) - cf2.centroid(i)) * (cf1.centroid(i) - cf2.centroid(i)));
    }
    sum += cf1.ssd + cf2.ssd;
    return sum > 0 ? sum * div : 0.;
  }
  ...
  public double matSelfInit(ClusteringFeature cf) {
    return 1 / cf.n * cf.sumOfSquaredDev();
  }
