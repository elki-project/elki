...
public class RadiusDistance implements BIRCHDistance {
  ...
  public double squaredDistance(NumberVector nv, ClusteringFeature cf1) {
    ...
    double sum = 0.;
    for(int i = 0; i < dim; i++) {
      sum += cf1.ssd[i][i] + cf1.n / (cf1.n + 1.) * ((cf1.centroid(i) - nv.doubleValue(i)) * (cf1.centroid(i) - nv.doubleValue(i)));
    }
  ...
  public double squaredDistance(ClusteringFeature cf1, ClusteringFeature cf2) {
    ...
    double sum = 0.;
    for(int i = 0; i < dim; i++) {
      sum += cf1.ssd[i][i] + cf2.ssd[i][i] + (n1 * n2) / (n12) * ((cf1.centroid(i) - cf2.centroid(i)) * (cf1.centroid(i) - cf2.centroid(i)));
    }
    return sum > 0 ? sum * div : 0.;
  }
