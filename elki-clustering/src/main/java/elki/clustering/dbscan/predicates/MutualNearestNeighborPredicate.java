...
/**
 * A neighbor predicate using the k mutual nearest neighbors, as used for
...
public class MutualNearestNeighborPredicate<O> implements NeighborPredicate<O, DBIDs> {
  /**
   ...
   */
  public MutualNearestNeighborPredicate(int k, Distance<? super O> distance) {
    super();
    this.kplus1 = k + 1;
  ...
  public Instance instantiate(Relation<? extends O> relation) {
    KNNSearcher<DBIDRef> knnq = new QueryBuilder<>(relation, distance).precomputed().kNNByDBID(kplus1);
  ...
  public static class Instance implements NeighborPredicate.Instance<DBIDs> {
    ...
    public DBIDs getNeighbors(DBIDRef reference) {
      ArrayModifiableDBIDs neighbors = DBIDUtil.newArray(kplus1);
      for(DoubleDBIDIter nn = knnq.getKNN(reference, kplus1).iter(); nn.valid(); nn.advance()) {
        // As we requested materialized kNN, this is stored.
        if(nn.doubleValue() <= knnq.getKNN(nn, kplus1).getKNNDistance()) {
          neighbors.add(nn);
        }
      }
      return neighbors;
  ...
  public static class Par<O> implements Parameterizer {
     ...
     */
    public static final OptionID DISTANCE_FUNCTION_ID = NearestNeighborPredicate.Par.DISTANCE_FUNCTION_ID;

    /**
     * Option id for the number of neighbors
     */
    public static final OptionID KNN_ID = NearestNeighborPredicate.Par.KNN_ID;
    ...
    @Override
    public MutualNearestNeighborPredicate<O> make() {
      return new MutualNearestNeighborPredicate<>(k, distfun);
    }
  }
}
