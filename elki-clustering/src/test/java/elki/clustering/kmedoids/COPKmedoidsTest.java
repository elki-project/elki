 ...
 * @author Andreas Lang
 */
public class COPKmedoidsTest extends AbstractClusterAlgorithmTest {
  @Test
  public void testKMedoidsPAMLabeled() {
    Database db = makeSimpleDatabase(UNITTEST + "different-densities-2d-no-noise.ascii", 1000);
    Clustering<MedoidModel> result = new ELKIBuilder<COPKmedoids<DoubleVector>>(COPKmedoids.class) //
        .with(KMeans.K_ID, 5) //
  ...
  public void testKMedoidsPAM() {
    ...
    Clustering<MedoidModel> result = new ELKIBuilder<LabeledPAM<DoubleVector>>(LabeledPAM.class) //
        ...
        .with(KMeans.SEED_ID, 0) //
        .build().autorun(db);
    // FMeasure compares the non Labels with labels
