 ...
 * @author Andreas Lang
 */
public class SeededKmedoidsTest extends AbstractClusterAlgorithmTest {
  @Test
  public void testKMedoidsPAMLabeled() {
    Database db = makeSimpleDatabase(UNITTEST + "different-densities-2d-no-noise.ascii", 1000);
    Clustering<MedoidModel> result = new ELKIBuilder<SeededKmedoids<DoubleVector>>(SeededKmedoids.class) //
        .with(KMeans.K_ID, 5) //
        .with(KMeans.SEED_ID, 0) //
        .build().autorun(db);
    assertFMeasure(db, result, 0.99800);
