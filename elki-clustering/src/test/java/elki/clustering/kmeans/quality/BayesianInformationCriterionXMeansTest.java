...
public class BayesianInformationCriterionXMeansTest extends AbstractClusterAlgorithmTest {
  ...
  public void testXMeans() {
    ...
    Clustering<?> result = new ELKIBuilder<XMeans<DoubleVector, ?>>(XMeans.class) //
        ...
        .with(KMeans.K_ID, 20) //
        .with(XMeans.Par.INFORMATION_CRITERION_ID, BayesianInformationCriterionXMeans.class) //
        .with(KMeans.SEED_ID, 1) // // Initializer seed
        .with(XMeans.Par.SEED_ID, 0) // // X-means seed
        .build().autorun(db);
    assertFMeasure(db, result, 0.959044);
    assertClusterSizes(result, new int[] { 1, 1, 2, 2, 2, 2, 2, 3, 5, 51, 106, 153 });
  }
}
