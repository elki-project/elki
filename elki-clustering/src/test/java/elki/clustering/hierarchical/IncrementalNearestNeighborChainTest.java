...
/**
 * Test the Incremental Nearest-Neighbor Chain (INNC) algorithm.
 *
 * @author Erich Schubert
 */
public class IncrementalNearestNeighborChainTest extends AbstractClusterAlgorithmTest {
  @Test
  public void testWard() {
    Database db = makeSimpleDatabase(UNITTEST + "single-link-effect.ascii", 638);
    Clustering<?> clustering = new ELKIBuilder<>(CutDendrogramByNumberOfClusters.class) //
        .with(CutDendrogramByNumberOfClusters.Par.MINCLUSTERS_ID, 3) //
        .with(Algorithm.Utils.ALGORITHM_ID, IncrementalNearestNeighborChain.class) //
        .with(IncrementalNearestNeighborChain.Par.LINKAGE_ID, WardLinkage.class) //
        .build().autorun(db);
  ...
  public void testCentroid() {
    ...
    Clustering<?> clustering = new ELKIBuilder<>(CutDendrogramByNumberOfClusters.class) //
        .with(CutDendrogramByNumberOfClusters.Par.MINCLUSTERS_ID, 3) //
        .with(Algorithm.Utils.ALGORITHM_ID, IncrementalNearestNeighborChain.class) //
        .with(IncrementalNearestNeighborChain.Par.LINKAGE_ID, CentroidLinkage.class) //
        .build().autorun(db);
    assertFMeasure(db, clustering, 0.93866265);
    assertClusterSizes(clustering, new int[] { 200, 211, 227 });
  }
  ...
  public void testMedian() {
    ...
    Clustering<?> clustering = new ELKIBuilder<>(CutDendrogramByNumberOfClusters.class) //
        .with(CutDendrogramByNumberOfClusters.Par.MINCLUSTERS_ID, 3) //
        .with(Algorithm.Utils.ALGORITHM_ID, IncrementalNearestNeighborChain.class) //
        .with(IncrementalNearestNeighborChain.Par.LINKAGE_ID, MedianLinkage.class) //
        .build().autorun(db);
