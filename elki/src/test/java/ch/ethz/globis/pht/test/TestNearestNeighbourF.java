...
public class TestNearestNeighbourF {
  ...
  private <T> PhTreeF<T> newTree(int DIM) {
	...
	public void testDirectHit() {
		...
		List<double[]> result = idx.nearestNeighbour(0, 3, 3);
	...
	public void testNeighbour1of4() {
		...
		List<double[]> result = idx.nearestNeighbour(1, 3, 3);
		check(8, result.get(0), 2, 2);
		assertEquals(1, result.size());
	}
	...
	public void testNeighbour4_5of4() {
		...
		List<double[]> result = idx.nearestNeighbour(4, 3, 3);
...
//				System.out.println("exp="+ Arrays.toString(exp));
				List<double[]> nnList = ind.nearestNeighbour(1, v);
...
//				System.out.println("exp=" + Arrays.toString(exp));
  ...
  public void testQueryND64RandomDF() {
    ...
    final Random R = new Random(0);
    for (int d = 0; d < LOOP; d++) {
      PhTreeF<Object> ind = PhTreeF.create(DIM);
      for (int i = 0; i < N; i++) {
        double[] v = new double[DIM];
        for (int j = 0; j < DIM; j++) {
          v[j] = R.nextDouble();
...
//        System.out.println("v="+ Arrays.toString(v));
//        System.out.println("exp="+ Arrays.toString(exp));
        List<double[]> nnList = ind.nearestNeighbour(1, PhDistanceD.THIS, v);
...
//        System.out.println("exp=" + Arrays.toString(exp));
	...
	public void testWrongResult() {
	  double[][] data = {
		...
		List<double[]> lst = ind.nearestNeighbour(1, center);
