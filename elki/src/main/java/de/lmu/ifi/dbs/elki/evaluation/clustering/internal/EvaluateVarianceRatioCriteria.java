...
public class EvaluateVarianceRatioCriteria<O> implements Evaluator {
  ...
  public double evaluateClustering(Database db, Relation<? extends NumberVector> rel, Clustering<?> c) {
    ...
    int clustercount = globalCentroid(overallCentroid, rel, clusters, centroids);
   ...
   */
  public int globalCentroid(Centroid overallCentroid, Relation<? extends NumberVector> rel, List<? extends Cluster<?>> clusters, NumberVector[] centroids) {
    int clustercount = 0;
    Iterator<? extends Cluster<?>> ci = clusters.iterator();
    for(int i = 0; ci.hasNext(); i++) {
      Cluster<?> cluster = ci.next();
