...
/**
 * Helper class to manage the assignment of points to nodes for the Cover Tree
 * k-means algorithms
 * <p>
 * Reference:
 * <p>
...
class NodeManager {
   ...
   */
  private static final Logging LOG = Logging.getLogger(NodeManager.class);

  /**
   * Enable additional (expensive) debug consistency checks.
   */
  ...
  /**
   * Assign point to new cluster without checks, when no tree is used anymore
   * (Hybrid)
   * 
   * @param id DBID Ref
   * @param fv Object
  ...
  public int testClean(Node n, int clu) {
    ...
    if(fails > 0) {
      LOG.warning("Tree is faulty because of Node");
    }
    for(Node c : n.children) {
      fails += testClean(c, clu);
    }
    if(fails > 0) {
      LOG.warning("Tree is faulty after children");
    }
    ...
    if(fails > 0) {
      LOG.warning("Tree is faulty after singletons");
    }
    return fails;
  }
  ...
  public int testAssign(Node n, int clu, double[][] means) {
    int invalid = 0;
    ModifiableDBIDs collect = DBIDUtil.newHashSet(n.size); // TODO use list?
